                 ...
                 <div class="info-boxes-container row row-joined mb-2 font2">
                    <div class="info-box info-box-icon-left col-lg-4">
                        ...
                        <div class="info-box-content">
                            <h4>Reporte por Agente de Aduanas</h4>
...
$sqtrp="SELECT e.CAGE,
   SUM(CASE extract(year from e.fnum) WHEN '2012' THEN $varianom ELSE 0 END) AS a2012, 
   SUM(CASE extract(year from e.fnum) WHEN '2013' THEN $varianom ELSE 0 END) AS a2013, 
   SUM(CASE extract(year from e.fnum) WHEN '2014' THEN $varianom ELSE 0 END) AS a2014, 
   SUM(CASE extract(year from e.fnum) WHEN '2015' THEN $varianom ELSE 0 END) AS a2015, 
   SUM(CASE extract(year from e.fnum) WHEN '2016' THEN $varianom ELSE 0 END) AS a2016,
   SUM(CASE extract(year from e.fnum) WHEN '2017' THEN $varianom ELSE 0 END) AS a2017,
   SUM(CASE extract(year from e.fnum) WHEN '2018' THEN $varianom ELSE 0 END) AS a2018,
   SUM(CASE extract(year from e.fnum) WHEN '2019' THEN $varianom ELSE 0 END) AS a2019,
   SUM(CASE extract(year from e.fnum) WHEN '2020' THEN $varianom ELSE 0 END) AS a2020,
   SUM(CASE extract(year from e.fnum) WHEN '2021' THEN $varianom ELSE 0 END) AS a2021,
	 SUM(CASE extract(year from e.fnum) WHEN '2022' THEN $varianom ELSE 0 END) AS a2022,
	 SUM(CASE extract(year from e.fnum) WHEN '2023' THEN $varianom ELSE 0 END) AS a2023
   FROM exportacion as e where extract(year from e.fnum) >= '2012' AND extract(year from e.fnum) <= '$aniogen' AND e.part_nandi='$code1' GROUP BY e.CAGE ORDER BY a2022 DESC ";
...
foreach($ggts as $dsx) {
	...
	$ddxlt = "SELECT d.idagente, d.agencia FROM agente AS d WHERE d.idagente='$listval1' ";
$rxtdpt = $conexpg -> prepare($ddxlt); 
$rxtdpt -> execute(); 
$txxs = $rxtdpt -> fetchAll(PDO::FETCH_OBJ); 
if($rxtdpt -> rowCount() > 0) { 
foreach($txxs as $dxpx) {
	$mesname1 = $dxpx -> idagente;
	$mesname = $dxpx -> agencia;
}
}else{
	$mesname = "-";
}
