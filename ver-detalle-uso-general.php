...
$sqtrp = "SELECT i.* FROM importacion AS i WHERE YEAR(i.fech_ingsi) = '$code2' AND MONTH(i.fech_ingsi) = '$code4' AND part_nandi='$code3' limit 1000 ";
$partslqt = $conexpg -> prepare($sqtrp); 
$partslqt -> execute(); 
$ggts = $partslqt -> fetchAll(PDO::FETCH_OBJ); 
if($partslqt -> rowCount() > 0) { 
...
foreach($txrr as $dffx) {	
   ...
   echo '<td>'.$impor30.'</td>';
	  ...
	  echo"</tr>";
					...
					<?php if($iteespact>='1000'){ ?> <br>  <p align="center">Hay mas registros encontrados <a href="descargar_excel_import.php?year=<?=$code2;?>&pk=<?=$code3;?>&flat=<?=$code4;?>" class="btn btn-dark btn-outline-dark" >Descargar todo en excel</a> </p> <?php  } ?>
