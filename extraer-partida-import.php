...
if(!isset($_POST['searchTerm'])){
   ...
   $stmt = $conexpg->prepare("SELECT partida_nandi, concat(partida_nandi,' - ',substring(partida_desc,1,50)) as partida_desc FROM productos WHERE origen_impor='SI' ORDER BY partida_desc LIMIT :limit");
   ...
   $stmt = $conexpg->prepare("SELECT partida_nandi, concat(partida_nandi,' - ',substring(partida_desc,1,50)) as partida_desc FROM productos WHERE concat(partida_nandi,' - ',substring(partida_desc,1,50)) like :partida_desc AND origen_impor='SI' ORDER BY partida_desc LIMIT :limit");
