		 ...
		 echo"<tfoot>
		  ...
		  //while ($fila1=pg_fetch_array($resultado1)) {
			...
			$items_sector = $items_sector + 1;
			  ...
			  /*consultamos la descripcion del sector*/
			 // $query_sector="SELECT se.sector FROM sector se WHERE se.sector = '$cod_secto'";
			  $query_sector="SELECT se.sector, se.tipo FROM sector se WHERE se.sector = '$cod_secto' and se.tipo='$cod_tipo' GROUP BY sector, tipo";
			  /*$res_sector = pg_query($query_sector) or die("Error en la Consulta SQL SECTOR");
	  $numReg_sector = pg_num_rows($res_sector);
	  if($numReg_sector>0){
		  while ($fila_sector=pg_fetch_array($res_sector)) {*/
...
while($fila_sector=$res_sector->fetch_array()){
	  ...
	  /*consultamos valor fob sumatoria*/
...
INNER JOIN sector ON sector.partida = exportacion.part_nandi
WHERE
sector.sector = '$cod_secto' AND sector.tipo='$cod_tipo'
GROUP BY
sector,tipo";
