...
$codeid = $_POST["namesectA"];
$namesector = $_POST["namesectNAME"];
$namedepa1 = $_POST["namedepa"];
$ubicod1 = $_POST["codubi"];
if($ubicod1!=""){
	$flatcod = $ubicod1;
	//$queryA = "AND SUBSTRING(exportacion.ubigeo,1,2) like '%$flatcod'";
	$queryA = "AND SUBSTRING(exportacion.ubigeo,1,2) = '$flatcod' ";
}else{
	$flatcod = "";
	$queryA = "";
}

$rangofecha = "extract(year from exportacion.fnum) >= '2012' AND extract(year from exportacion.fnum) <= '2021' ";
...
<div class="loader"></div>
    ...
    <!--<div class="wrapper wrapper-full-page">-->
          ...
          <div class="alert alert-warning alert-with-icon" data-notify="container">
		   ...
		   FROM exportacion INNER JOIN sector ON sector.partida = exportacion.part_nandi  WHERE $rangofecha $queryA  AND concat(sector.tipo,sector.sector) ='".$codeid."'";
...
while($fila_vfob=$resultado_vfob->fetch_array()){
	...
	$vfob_2021=  $fila_vfob['vfob2021'];
		//echo "paso uno";
			   ...
			   $var1= number_format((($vfob_2020/$vfob_2020) - 1) * 100,2);
			   }
...
$var3= number_format((($vpes_2020/$vpes_2021) - 1) * 100,2);
			   }
...
while($fila_qunicom=$resultado_qunicom->fetch_array()){
	...
	$qunicom_2021= $fila_qunicom['quni2021'];
			 ...
			 if($qunicom_2016=='0'){
		      ...
		      $ca8_3 = (($qunicom_2020 / $qunicom_2017) - 1) * 100;
		      }
	
	if($qunicom_2020=='0'){
			  $ca9_3 ='0';
		      }else{
		      $ca9_3 = (($qunicom_2021 / $qunicom_2018) - 1) * 100;
		      }
...
while($fila_creg=$resultado_creg->fetch_array()){
	...
	$creg_2021= "0.00";
	  ...
	  $query_parti = "SELECT Count(DISTINCT exportacion.part_nandi) as cant_parti, extract(year from exportacion.fnum) as anio FROM exportacion INNER JOIN sector ON sector.partida = exportacion.part_nandi  WHERE $rangofecha $queryA AND concat(sector.tipo,sector.sector) ='".$codeid."' GROUP BY anio order BY anio ASC";
...
while($fila_parti=$resultado_parti->fetch_array()){
	...
	$rparti2021= "0.00";
	  ...
	  /*cuenta cantidad de duas*/
	   $query_dua = "SELECT Count(DISTINCT exportacion.ndcl) as cant_dua, extract(year from exportacion.fnum) as anio FROM exportacion INNER JOIN sector ON sector.partida = exportacion.part_nandi  WHERE $rangofecha $queryA AND concat(sector.tipo,sector.sector) ='".$codeid."' GROUP BY anio order BY anio ASC";
...
while($fila_dua=$resultado_dua->fetch_array()){
	...
	$rdua2021= "0.00";
	  ...
	  /*cuenta cantidad de empresas*/
	   $query_emp = "SELECT Count(DISTINCT exportacion.dnombre) as cant_nom, extract(year from exportacion.fnum) as anio FROM exportacion INNER JOIN sector ON sector.partida = exportacion.part_nandi WHERE $rangofecha $queryA AND concat(sector.tipo,sector.sector) ='".$codeid."' GROUP BY anio order BY anio ASC";
...
while($fila_emp=$resultado_emp->fetch_array()){
	...
	$remp2021= "0.00";
	 ...
	 $query_merca = "SELECT Count(DISTINCT exportacion.cpaides) as cant_mer, extract(year from exportacion.fnum) as anio FROM exportacion INNER JOIN sector ON sector.partida = exportacion.part_nandi  WHERE $rangofecha $queryA AND concat(sector.tipo,sector.sector) ='".$codeid."' GROUP BY anio order BY anio ASC";
...
while($fila_merca=$resultado_merca->fetch_array()){
	...
	$rmer2021= "0.00";
	  ...
	  /*contamos cantidad de puertos*/
	   $query_pue = "SELECT Count(DISTINCT exportacion.cpuedes) as cant_pue, extract(year from exportacion.fnum) as anio FROM exportacion INNER JOIN sector ON sector.partida = exportacion.part_nandi  WHERE $rangofecha $queryA AND concat(sector.tipo,sector.sector) ='".$codeid."' GROUP BY anio order BY anio ASC";
...
while($fila_pue=$resultado_pue->fetch_array()){
	...
	$rpue2021= "0.00";
	  ...
	  /*contamos cantidad de aduanas*/
	   $query_adua = "SELECT Count(DISTINCT exportacion.cadu) as cant_adua, extract(year from exportacion.fnum) as anio FROM exportacion INNER JOIN sector ON sector.partida = exportacion.part_nandi  WHERE $rangofecha $queryA AND concat(sector.tipo,sector.sector) ='".$codeid."' GROUP BY anio order BY anio ASC";
...
while($fila_adua=$resultado_adua->fetch_array()){
	...
	$radua2021= "0.00";
	  ...
	  /*contamnos cantidad de departamento*/
	   $query_empresa = "SELECT COUNT(DISTINCT departamento.nombre) as departamento, extract(year from exportacion.fnum) as anio FROM exportacion LEFT JOIN departamento ON  ((((SUBSTRING(exportacion.ubigeo,1,2))= departamento.iddepartamento))) INNER JOIN sector ON sector.partida = exportacion.part_nandi  WHERE $rangofecha $queryA AND concat(sector.tipo,sector.sector) ='".$codeid."' GROUP BY anio ORDER BY anio ASC";
...
extract(year from exportacion.fnum) <= '2018' AND substring(exportacion.ubigeo,1,2) = '".$codeid."' GROUP BY anio ORDER BY anio ASC";
	...
	$vardist2= number_format(($ca1_dis + $ca2_dis + $ca3_dis + $ca4_dis + $ca5_dis)/5,2) ;
	  ...
	  /*contamnos cantidad de agente*/
	   $query_agen = "SELECT Count(DISTINCT exportacion.cage) as cant_agen, extract(year from exportacion.fnum) as anio FROM exportacion INNER JOIN sector ON sector.partida = exportacion.part_nandi WHERE $rangofecha $queryA AND concat(sector.tipo,sector.sector) ='".$codeid."' GROUP BY anio order BY anio ASC";
...
while($fila_agen=$resultado_agen->fetch_array()){
	...
	$ragen2021= "0.00";
	  ...
	  /*contamos cantidad via de transporte*/
	   $query_via = "SELECT Count(DISTINCT exportacion.cviatra) as cant_via, extract(year from exportacion.fnum) as anio FROM exportacion INNER JOIN sector ON sector.partida = exportacion.part_nandi WHERE $rangofecha $queryA AND concat(sector.tipo,sector.sector) ='".$codeid."' GROUP BY anio order BY anio ASC";
...
echo"<div class='col-md-12'>";
	...
	echo"<div class='card-header card-header-info card-header-icon'>
              ...
              <h4 class='card-title'><b>Reporte Analisis Sector Indicador Anual - Exportaciones </b><br> Sector: $namesector │ Departamento: $namedepa1 │ Fecha Numeracion │ Periodo 2012 - 2021 </b> </h4>
            </div>";
...
<script type="text/javascript" src="https://www.gstatic.com/charts/loader.js"></script>
    ...
    var options = {
      title : 'Reporte Analisis Anual Sector <?php echo $namesector ?> -  Departamento: <?=$namedepa1;?> - Exportaciones',
      vAxis: {title: 'Cups'},
      hAxis: {title: 'Years'},
      seriesType: 'bars',
      series: {6: {type: 'line'}}
