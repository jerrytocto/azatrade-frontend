...
LEFT JOIN aduana ON exportacion.cadu = aduana.codadu 
INNER JOIN sector ON sector.partida = exportacion.part_nandi 
WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND ".$wherefecha." GROUP BY exportacion.cadu,aduana.descripcion ";
...
LEFT JOIN aduana ON exportacion.cadu = aduana.codadu
INNER JOIN sector ON sector.partida = exportacion.part_nandi 
WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND ".$wherefecha." GROUP BY exportacion.cadu,aduana.descripcion ORDER BY xx2019 DESC LIMIT 5";
  }else if($nombres2=="total"){
	  $query_resultado = "SELECT
exportacion.cadu,
aduana.descripcion,
...
LEFT JOIN aduana ON exportacion.cadu = aduana.codadu
INNER JOIN sector ON sector.partida = exportacion.part_nandi
WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND ".$wherefecha." GROUP BY exportacion.cadu,aduana.descripcion ORDER BY a2020 DESC LIMIT 5";
  }else{
	  if($nombres2=="provi"){
		  $variaA='substring(exportacion.ubigeo,3,2)';
	  }else{
...
?>
      ...
      var options = {
        title: 'Sector <?=$namesector;?> - Departamento: <?=$namedepa1;?> - Evolución Anual  <?php echo $combo ?> <?php echo $combo2 ?> de Exportaciones Periodo 2012 - 2021',
        chartArea: {width: '50%'},
        isStacked: true,
        hAxis: {
