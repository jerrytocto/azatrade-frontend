...
FROM exportacion
LEFT JOIN paises ON exportacion.cpaides = paises.idpaises INNER JOIN sector ON sector.partida = exportacion.part_nandi  WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND ".$wherefecha." ";
  }
  if($nombres2=="diferen"){
	  $sqlyear="SELECT
...
FROM exportacion
LEFT JOIN paises ON exportacion.cpaides = paises.idpaises INNER JOIN sector ON sector.partida = exportacion.part_nandi 
...
LEFT JOIN paises ON exportacion.cpaides = paises.idpaises INNER JOIN sector ON sector.partida = exportacion.part_nandi 
WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND ".$wherefecha."
GROUP BY exportacion.cpaides,paises.espanol ";
...
FROM exportacion
LEFT JOIN paises ON exportacion.cpaides = paises.idpaises INNER JOIN sector ON sector.partida = exportacion.part_nandi 
WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND ".$wherefecha." GROUP BY exportacion.cpaides,paises.espanol ";
...
".$wherefecha." GROUP BY exportacion.cpaides,paises.espanol ";
...
echo"<div class='col-md-12'>";
	...
	echo"<div class='card-header card-header-info card-header-icon'>
              ...
              <h4 class='card-title'><b>Reporte Sector Evoluci&oacute;n Anual de Exportaciones</b><br> Sector: $namesector │ Departamento: $namedepa1 │ Indicador #1: $combo - Variable #2: $combo2 │ Fecha Numeracion │ Periodo 2012 - 2021 </b> </h4>
            </div>";
...
LEFT JOIN paises ON exportacion.cpaides = paises.idpaises
INNER JOIN sector ON sector.partida = exportacion.part_nandi 
WHERE concat(sector.tipo,sector.sector)= '".$codeid."' $querybusca AND ".$wherefecha." GROUP BY exportacion.cpaides,paises.descripcion ";
  }else if($nombres2=="total"){
	  $query_resultado = "SELECT
exportacion.cpaides,
paises.descripcion,
...
while($fila_rpt=$resultado_rpt->fetch_array()){
  ...
  <center>
        <h3 class="title" id="sector"><font color="#FFFFFF">Mapa Geografico │ Sector <?=$namesector;?> │  Departamento: <?=$namedepa1;?> │ Evolucion Anual de Exportaciones │ <?=$combo;?> │ <?=$combo2;?> │ Periodo 2020</font></h3></center>
