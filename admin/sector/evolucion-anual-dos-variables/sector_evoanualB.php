...
FROM exportacion
INNER JOIN sector ON sector.partida = exportacion.part_nandi
WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND ".$wherefecha." GROUP BY exportacion.dnombre ORDER BY exportacion.dnombre ASC";
...
".$wherefecha." GROUP BY exportacion.dnombre ORDER BY exportacion.dnombre ASC";
...
INNER JOIN sector ON sector.partida = exportacion.part_nandi
WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND ".$wherefecha."
GROUP BY exportacion.dnombre ORDER BY a2019 DESC LIMIT 5";
  }else if ($nombres2=="diferen"){
	  $query_resultado = "SELECT
exportacion.dnombre,
Sum(CASE ".$filtrofecha." WHEN '2012' THEN exportacion.vfobserdol ELSE 0 END) AS x2012,
...
FROM exportacion
INNER JOIN sector ON sector.partida = exportacion.part_nandi 
WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND ".$wherefecha." GROUP BY exportacion.dnombre ORDER BY xx2020 DESC LIMIT 5";
  }else if($nombres2=="total"){
	  $query_resultado = "SELECT
exportacion.dnombre,
Sum(CASE ".$filtrofecha." WHEN '2012' THEN 1 ELSE 0 END) AS a2012,
...
FROM exportacion
INNER JOIN sector ON sector.partida = exportacion.part_nandi 
WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND ".$wherefecha." GROUP BY exportacion.dnombre ORDER BY a2020 DESC LIMIT 5";
  } else if($nombres2=="part_nandi" or $nombres2=="ndcl" or $nombres2=="dnombre" or $nombres2=="cpaides" or $nombres2=="cpuedes" or  $nombres2=="cadu" or $nombres2=="cage" or $nombres2=="cviatra"){
	  $query_resultado = "SELECT
exportacion.dnombre,
COUNT(distinct CASE ".$filtrofecha." WHEN '2012' THEN  exportacion.".$nombres2." ELSE NULL END) AS a2012,
