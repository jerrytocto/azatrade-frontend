...
set_time_limit(700);
//$namedato = $_POST["namedepaD"];
$codeid = $_POST["namesectD"];
...
FROM exportacion
INNER JOIN sector ON sector.partida = exportacion.part_nandi WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND $wherefecha ";
  }
  if($nombres2=="diferen"){
	  $sqlyear="SELECT
...
FROM exportacion
INNER JOIN sector ON sector.partida = exportacion.part_nandi
WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND $wherefecha GROUP BY exportacion.part_nandi ";
  }else if ($nombres2=="diferen"){
	  $query_resultado = "SELECT
exportacion.part_nandi,
...
echo"<div class='col-md-12'>";
	...
	echo"<div class='card-header card-header-info card-header-icon'>
              ...
              <h4 class='card-title'><b>Reporte Sector Evolucion Anual de Exportaciones</b><br> Sector: $namesector │ Departamento: $namedepa1 │ Indicador #1: $combo - Variable #2: $combo2 │ Fecha Numeracion │ Periodo 2012 - 2021 </b> </h4>
            </div>";
...
INNER JOIN sector ON sector.partida = exportacion.part_nandi
WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND ".$wherefecha."
GROUP BY exportacion.part_nandi ORDER BY a2020 DESC LIMIT 5";
  }else if ($nombres2=="diferen"){
	  $query_resultado = "SELECT
exportacion.part_nandi,
...
FROM exportacion
INNER JOIN sector ON sector.partida = exportacion.part_nandi
WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND ".$wherefecha." GROUP BY exportacion.part_nandi,arancel.descripcion ORDER BY xx2020 DESC LIMIT 5";
  }else if($nombres2=="total"){
	  $query_resultado = "SELECT
exportacion.part_nandi,
...
FROM exportacion
INNER JOIN sector ON sector.partida = exportacion.part_nandi
WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND ".$wherefecha." GROUP BY exportacion.part_nandi ORDER BY a2020 DESC LIMIT 5";
  } else if($nombres2=="part_nandi" or $nombres2=="ndcl" or $nombres2=="dnombre" or $nombres2=="cpaides" or $nombres2=="cpuedes" or  $nombres2=="cadu" or $nombres2=="cage" or $nombres2=="cviatra"){
	  $query_resultado = "SELECT
exportacion.part_nandi,
...
?>
      ...
      var options = {
        title: 'Sector: <?php echo $namesector ?> - Departamento: <?=$namedepa1;?> - Evolución Anual <?php echo $combo ?> <?php echo $combo2 ?> de Exportaciones Periodo 2012 - 2021',
        chartArea: {width: '50%'},
        isStacked: true,
        hAxis: {
...
include("sector_evoanualA.php");
}

if($nombres3=="empresaexpo"){//consulta como variable empresas exportadoras
include("sector_evoanualB.php");
}

if($nombres3=="pais"){//consulta como variable pais
include("sector_evoanualC.php");
}

if($nombres3=="agente"){
	include("sector_evoanualD.php");
}
  
  if($nombres3=="aduanas"){
 include("sector_evoanualE.php");
  }
  if($nombres3=="ubigeo"){
  include("sector_evoanualF.php");
  }
				?>
