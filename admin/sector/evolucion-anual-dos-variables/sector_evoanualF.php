...
substring(exportacion.ubigeo,1,2) as ubigeo,
Sum(CASE ".$filtrofecha." WHEN '2012' THEN ".$nombres2." ELSE 0 END) AS a2012,
Sum(CASE ".$filtrofecha." WHEN '2013' THEN ".$nombres2." ELSE 0 END) AS a2013,
Sum(CASE ".$filtrofecha." WHEN '2014' THEN ".$nombres2." ELSE 0 END) AS a2014,
Sum(CASE ".$filtrofecha." WHEN '2015' THEN ".$nombres2." ELSE 0 END) AS a2015,
...
INNER JOIN sector ON sector.partida = exportacion.part_nandi
WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND ".$wherefecha."
GROUP BY substring(exportacion.ubigeo,1,2) ";
...
FROM exportacion
INNER JOIN sector ON sector.partida = exportacion.part_nandi 
WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND ".$wherefecha." GROUP BY substring(exportacion.ubigeo,1,2)";
...
FROM exportacion
INNER JOIN sector ON sector.partida = exportacion.part_nandi 
WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND
".$wherefecha." GROUP BY  substring(exportacion.ubigeo,1,2) ";
...
echo"<div class='col-md-12'>";
	...
	echo"<div class='card-header card-header-info card-header-icon'>
              ...
              <h4 class='card-title'><b>Reporte Sector Evoluci&oacute;n Mensual de Exportaciones</b><br> Sector: $namesector │ Departamento: $namedepa1 │ Variable #1: $combo - Variable #2: $combo2 │ Fecha Numeracion │ Periodo 2012 - 2021 </b> </h4>
            </div>";
		...
		echo"<table id='datatablese' class='table table-striped table-no-bordered table-hover' cellspacing='0' width='100%' style='width:100%'>";
		 ...
		 // while ($fila_rpt=pg_fetch_array($resultado_rpt)) {
		   ...
		   $codigotemp= $fila_rpt['ubigeo'];
		   
		   /*consultamos departamentos segun codigo*/
		   $sql_depa="SELECT substring(ubigeo.cubigeo,1,2), ubigeo.nombredepartamento FROM ubigeo WHERE substring(ubigeo.cubigeo,1,2)='$codigotemp' GROUP BY substring(ubigeo.cubigeo,1,2), ubigeo.nombredepartamento";
		   /*$resu_depa = pg_query($sql_depa) or die("Error en la Consulta SQL Reporte Ubigeo");
	  $num_depa = pg_num_rows($resu_depa);
	  if($num_depa>0){
		   while ($fila_depa=pg_fetch_array($resu_depa)) {*/
				  $resu_depa=$conexpg->query($sql_depa); 
if($resu_depa->num_rows>0){ 
while($fila_depa=$resu_depa->fetch_array()){
	
		   $nombredesc= $fila_depa['nombredepartamento'];
		   if($nombredesc==""){
			   $nombredesc="---";
		   }else{
			   $nombredesc= $fila_depa['nombredepartamento'];
	  ...
	  /* fin consultamos departamentos segun codigo*/
		   
		  /* $nombredesc= $fila_rpt['nombredepartamento'];
		   if($nombredesc==""){
			   $nombredesc="---";
		   }else{
			   $nombredesc= $fila_rpt['nombredepartamento'];
...
INNER JOIN sector ON sector.partida = exportacion.part_nandi 
WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND ".$wherefecha."
GROUP BY substring(exportacion.ubigeo,1,2) ORDER BY a2020 DESC";
  }else if ($nombres2=="diferen"){
	  $query_resultado = "SELECT
substring(exportacion.ubigeo,1,2) as ubigeo,
Sum(CASE ".$filtrofecha." WHEN '2012' THEN exportacion.vfobserdol ELSE 0 END) AS x2012,
...
FROM exportacion
INNER JOIN sector ON sector.partida = exportacion.part_nandi 
WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND ".$wherefecha." GROUP BY substring(exportacion.ubigeo,1,2) ORDER BY xx2020 DESC";
  }else if($nombres2=="total"){
	  $query_resultado = "SELECT
substring(exportacion.ubigeo,1,2) as ubigeo,
Sum(CASE ".$filtrofecha." WHEN '2012' THEN 1 ELSE 0 END) AS a2012,
...
FROM exportacion
INNER JOIN sector ON sector.partida = exportacion.part_nandi 
WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND ".$wherefecha." GROUP BY substring(exportacion.ubigeo,1,2) ORDER BY a2020 DESC";
  } else if($nombres2=="part_nandi" or $nombres2=="ndcl" or $nombres2=="dnombre" or $nombres2=="cpaides" or $nombres2=="cpuedes" or  $nombres2=="cadu" or $nombres2=="cage" or $nombres2=="cviatra"){
	  $query_resultado = "SELECT
substring(exportacion.ubigeo,1,2) as ubigeo,
COUNT(distinct CASE ".$filtrofecha." WHEN '2012' THEN  exportacion.".$nombres2." ELSE NULL END) AS a2012,
...
INNER JOIN sector ON sector.partida = exportacion.part_nandi
WHERE concat(sector.tipo,sector.sector) = '".$codeid."' $querybusca AND
".$wherefecha." GROUP BY substring(exportacion.ubigeo,1,2),ubigeo.nombredepartamento ORDER BY a2020 DESC ";
  }
			$resultado_rpt=$conexpg->query($query_resultado); 
if($resultado_rpt->num_rows>0){ 
while($fila_rpt=$resultado_rpt->fetch_array()){
...
?>
     ...
     <center>
        <h3 class="title" id="mapa"><font color="#FFFFFF">Mapa Geografico │ Sector <?=$namesector;?> │ Departamento: <?=$namedepa1;?> │ Departamento Anual de Exportaciones │ <?=$combo;?> │ <?=$combo2;?> │ Periodo 2020</font></h3></center>
