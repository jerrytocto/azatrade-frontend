...
FROM exportacion
LEFT JOIN puerto ON exportacion.cpuedes = puerto.idcodigo WHERE substring(exportacion.ubigeo,1,2) = '".$codeid."' AND ".$wherefecha." ";
  }
  if($nombres2=="diferen"){
	  $sqlyear="SELECT
...
FROM exportacion
LEFT JOIN puerto ON exportacion.cpuedes = puerto.idcodigo
WHERE substring(exportacion.ubigeo,1,2) = '".$codeid."' AND ".$wherefecha." GROUP BY exportacion.cpuedes,puerto.puerto ";
...
FROM exportacion
LEFT JOIN puerto ON exportacion.cpuedes = puerto.idcodigo
WHERE substring(exportacion.ubigeo,1,2) = '".$codeid."' AND ".$wherefecha." GROUP BY exportacion.cpuedes,puerto.puerto ORDER BY xx2020 DESC LIMIT 5";
  }else if($nombres2=="total"){
	  $query_resultado = "SELECT
exportacion.cpuedes,
puerto.puerto,
...
exportacion
LEFT JOIN puerto ON exportacion.cpuedes = puerto.idcodigo
WHERE substring(exportacion.ubigeo,1,2) = '".$codeid."' AND ".$wherefecha." GROUP BY exportacion.cpuedes,puerto.puerto ORDER BY a2020 DESC LIMIT 5";
  }else{
	  if($nombres2=="provi"){
		  $variaA='substring(exportacion.ubigeo,3,2)';
	  }else{
...
?>
      ...
      var options = {
        title: 'Ubigeo: <?php echo $namedato ?> - Evolución Mensual <?php echo $combo ?> <?php echo $combo2 ?> de Exportaciones Periodo 2012 - 2021',
        chartArea: {width: '50%'},
        isStacked: true,
        hAxis: {
