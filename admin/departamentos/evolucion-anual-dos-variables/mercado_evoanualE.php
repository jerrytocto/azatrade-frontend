...
FROM exportacion
LEFT JOIN aduana ON exportacion.cadu = aduana.codadu WHERE substring(exportacion.ubigeo,1,2) = '".$codeid."' AND ".$wherefecha." ";
  }
  if($nombres2=="diferen"){
	  $sqlyear="SELECT
...
FROM exportacion
LEFT JOIN aduana ON exportacion.cadu = aduana.codadu
WHERE substring(exportacion.ubigeo,1,2) = '".$codeid."' AND ".$wherefecha." GROUP BY exportacion.cadu,aduana.descripcion ";
...
FROM exportacion
LEFT JOIN aduana ON exportacion.cadu = aduana.codadu
WHERE substring(exportacion.ubigeo,1,2) = '".$codeid."' AND ".$wherefecha." GROUP BY exportacion.cadu,aduana.descripcion ORDER BY xx2020 DESC LIMIT 5";
  }else if($nombres2=="total"){
	  $query_resultado = "SELECT
exportacion.cadu,
aduana.descripcion,
...
LEFT JOIN aduana ON exportacion.cadu = aduana.codadu
WHERE substring(exportacion.ubigeo,1,2) = '".$codeid."' AND
".$wherefecha." GROUP BY exportacion.cadu,aduana.descripcion ORDER BY a2019 DESC LIMIT 5";
  } else if($nombres2=="part_nandi" or $nombres2=="ndcl" or $nombres2=="dnombre" or $nombres2=="cpaides" or $nombres2=="cpuedes" or  $nombres2=="cadu" or $nombres2=="cage" or $nombres2=="cviatra"){
	  $query_resultado = "SELECT
exportacion.cadu,
aduana.descripcion,
...
exportacion
LEFT JOIN aduana ON exportacion.cadu = aduana.codadu
WHERE substring(exportacion.ubigeo,1,2) = '".$codeid."' AND ".$wherefecha." GROUP BY exportacion.cadu,aduana.descripcion ORDER BY a2020 DESC LIMIT 5";
  }else{
	  if($nombres2=="provi"){
		  $variaA='substring(exportacion.ubigeo,3,2)';
	  }else{
...
?>
      ...
      var options = {
        title: 'Ubigeo: <?=$namedato;?> Evolución Mensual <?php echo $combo ?> <?php echo $combo2 ?> de Exportaciones Periodo 2012 - 2021',
        chartArea: {width: '50%'},
        isStacked: true,
        hAxis: {
