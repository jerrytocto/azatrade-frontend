...
FROM exportacion
WHERE substring(exportacion.ubigeo,1,2) = '".$codeid."' AND ".$wherefecha." GROUP BY exportacion.cage ";
...
".$wherefecha." GROUP BY exportacion.cage ";
...
echo"<div class='col-md-12'>";
	...
	echo"<div class='card-header card-header-info card-header-icon'>
              ...
              <h4 class='card-title'><b>Reporte Ubigeo Evoluci&oacute;n Anual de Exportaciones</b><br> Departamento: $namedato │ Variable #1: $combo - Variable #2: $combo2 │ Fecha Numeracion │ Periodo 2012 - 2021 </b> </h4>
            </div>";
 ...
 //while ($fila_rpt=pg_fetch_array($resultado_rpt)) {
	 ...
	 $cuentaD = $cuentaD + 1;
		 ...
		 $resadu=$conexpg->query($query_resultado); 
...
FROM exportacion
WHERE substring(exportacion.ubigeo,1,2) = '".$codeid."' AND ".$wherefecha."
GROUP BY exportacion.cage ORDER BY a2020 DESC LIMIT 5";
  }else if ($nombres2=="diferen"){
	  $query_resultado = "SELECT
exportacion.cage,
Sum(CASE ".$filtrofecha." WHEN '2012' THEN exportacion.vfobserdol ELSE 0 END) AS x2012,
...
Sum(CASE ".$filtrofecha." WHEN '2021' THEN exportacion.vpesnet ELSE 0 END) AS xx2021
FROM exportacion
WHERE substring(exportacion.ubigeo,1,2) = '".$codeid."' AND ".$wherefecha." GROUP BY exportacion.cage ORDER BY xx2019 DESC LIMIT 5";
  }else if($nombres2=="total"){
	  $query_resultado = "SELECT
exportacion.cage,
Sum(CASE ".$filtrofecha." WHEN '2012' THEN 1 ELSE 0 END) AS a2012,
...
FROM exportacion
WHERE substring(exportacion.ubigeo,1,2) = '".$codeid."' AND
".$wherefecha." GROUP BY exportacion.cage ORDER BY a2019 DESC LIMIT 5";
  } else if($nombres2=="part_nandi" or $nombres2=="ndcl" or $nombres2=="dnombre" or $nombres2=="cpaides" or $nombres2=="cpuedes" or  $nombres2=="cadu" or $nombres2=="cage" or $nombres2=="cviatra"){
	  $query_resultado = "SELECT
exportacion.cage,
COUNT(distinct CASE ".$filtrofecha." WHEN '2012' THEN  exportacion.".$nombres2." ELSE NULL END) AS a2012,
...
COUNT(distinct CASE ".$filtrofecha." WHEN '2021' THEN  exportacion.".$nombres2." ELSE NULL END) AS a2021
FROM exportacion
WHERE substring(exportacion.ubigeo,1,2) = '".$codeid."' AND ".$wherefecha." GROUP BY exportacion.cage ORDER BY a2019 DESC LIMIT 5";
  }else{
	  if($nombres2=="provi"){
		  $variaA='substring(exportacion.ubigeo,3,2)';
	  }else{
...
FROM exportacion
WHERE substring(exportacion.ubigeo,1,2) = '".$codeid."' AND
".$wherefecha." GROUP BY exportacion.cage ORDER BY a2020 DESC LIMIT 5";
  }
...
?>
		 ...
		 /*['New York City, NY', 8175000, 8008000,3005006,3005006,3005006],
        ['Los Angeles, CA', 3792000, 3694000,8022000,3005006,3005006],
        ['Chicago, IL', 2695000, 2896000,3789880,3005006,3005006],
        ['Houston, TX', 2099000, 1953000, 7899900,3005006,3005006],
        ['Philadelphia, PA', 1526000, 1517000, 2140000,3005006,3005006]*/
      ]);

      var options = {
        title: 'Ubigeo: <?=$namedato;?> Evolución Anual <?php echo $combo ?> <?php echo $combo2 ?> de Exportaciones Periodo 2012 - 2021',
        chartArea: {width: '50%'},
        isStacked: true,
        hAxis: {
