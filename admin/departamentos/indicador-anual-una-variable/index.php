...
set_time_limit(700);
$namedato = $_POST["namedepaC"];
$codeid = $_POST["codedepaC"];
$onevar = $_POST["unavari"];
$condi1 = "AND extract(year from exportacion.fnum) >= '2012' AND
extract(year from exportacion.fnum) <= '2021'";
$condiA1 = "extract(year from exportacion.fnum) >= '2012' AND
extract(year from exportacion.fnum) <= '2021'";
...
/*realizamos conusulta del reporte selecionado y  actualizamos en la tabla temporal crada*/
 ...
 WHERE substring(exportacion.ubigeo,1,2)='".$codeid."' $condi1 GROUP BY exportacion.fano,mes ORDER BY exportacion.fano,mes";
  }
  if($onevar=="vpesnet"){
  $query1 = "SELECT exportacion.fano, extract(MONTH from exportacion.fnum) as mes, Sum(exportacion.vpesnet) as resultado FROM exportacion WHERE substring(exportacion.ubigeo,1,2) = '".$codeid."' AND extract(year from exportacion.fnum) >= '2012' AND extract(year from exportacion.fnum) <= '2019' GROUP BY exportacion.fano,mes ORDER BY exportacion.fano,mes";
  }
  if($onevar=="diferen"){
  $query1 = "SELECT exportacion.fano, extract(MONTH from exportacion.fnum) as mes, Sum(exportacion.vfobserdol) / Sum(exportacion.vpesnet) as resultado FROM exportacion WHERE substring(exportacion.ubigeo,1,2) = '".$codeid."' $condi1 GROUP BY exportacion.fano,mes ORDER BY exportacion.fano,mes";
  }
  if($onevar=="vpesbru"){
  $query1 = "SELECT exportacion.fano, extract(MONTH from exportacion.fnum) as mes, Sum(exportacion.vpesbru) as resultado FROM exportacion WHERE substring(exportacion.ubigeo,1,2) = '".$codeid."' $condi1 GROUP BY exportacion.fano,mes ORDER BY exportacion.fano,mes";
  }
   if($onevar=="qunifis"){
  $query1 = "SELECT exportacion.fano, extract(MONTH from exportacion.fnum) as mes, Sum(exportacion.qunifis) as resultado FROM exportacion WHERE substring(exportacion.ubigeo,1,2) = '".$codeid."' $condi1 GROUP BY exportacion.fano,mes ORDER BY exportacion.fano,mes";
  }
  if($onevar=="qunicom"){
  $query1 = "SELECT exportacion.fano, extract(MONTH from exportacion.fnum) as mes, Sum(exportacion.qunicom) as resultado FROM exportacion WHERE substring(exportacion.ubigeo,1,2) = '".$codeid."' $condi1 GROUP BY exportacion.fano,mes ORDER BY exportacion.fano,mes";
  }
  if($onevar=="part_nandi"){
	  $query1 = "SELECT exportacion.fano, extract(MONTH from exportacion.fnum) as mes,COUNT(DISTINCT exportacion.part_nandi) as resultado FROM exportacion WHERE substring(exportacion.ubigeo,1,2) = '".$codeid."' $condi1 GROUP BY exportacion.fano,mes ORDER BY exportacion.fano,mes";
  }
  if($onevar=="total"){
  $query1 = "SELECT exportacion.fano, extract(MONTH from exportacion.fnum) as mes, count(*) as resultado FROM exportacion WHERE substring(exportacion.ubigeo,1,2) = '".$codeid."' $condi1 GROUP BY exportacion.fano,mes ORDER BY exportacion.fano,mes";
  }
  if($onevar=="ndcl"){
  $query1 = "SELECT exportacion.fano, extract(MONTH from exportacion.fnum) as mes,COUNT(DISTINCT exportacion.ndcl) as resultado FROM exportacion WHERE substring(exportacion.ubigeo,1,2) = '".$codeid."' $condi1 GROUP BY exportacion.fano,mes ORDER BY exportacion.fano,mes";
  }
  if($onevar=="dnombre"){
  $query1 = "SELECT exportacion.fano, extract(MONTH from exportacion.fnum) as mes,COUNT(DISTINCT exportacion.dnombre) as resultado FROM exportacion WHERE substring(exportacion.ubigeo,1,2) = '".$codeid."' $condi1 GROUP BY exportacion.fano,mes ORDER BY exportacion.fano,mes";
  }
  if($onevar=="cpaides"){
  $query1 = "SELECT exportacion.fano, extract(MONTH from exportacion.fnum) as mes,COUNT(DISTINCT exportacion.cpaides) as resultado FROM exportacion WHERE substring(exportacion.ubigeo,1,2) = '".$codeid."' $condi1 GROUP BY exportacion.fano,mes ORDER BY exportacion.fano,mes";
  }
  if($onevar=="cpuedes"){
  $query1 = "SELECT exportacion.fano, extract(MONTH from exportacion.fnum) as mes,COUNT(DISTINCT exportacion.cpuedes) as resultado FROM exportacion WHERE substring(exportacion.ubigeo,1,2) = '".$codeid."' $condi1 GROUP BY exportacion.fano,mes ORDER BY exportacion.fano,mes";
  }
  if($onevar=="cadu"){
  $query1 = "SELECT exportacion.fano, extract(MONTH from exportacion.fnum) as mes,COUNT(DISTINCT exportacion.cadu) as resultado FROM exportacion WHERE substring(exportacion.ubigeo,1,2) = '".$codeid."' $condi1 GROUP BY exportacion.fano,mes ORDER BY exportacion.fano,mes";
  }
   if($onevar=="depa"){
  $query1 = "SELECT exportacion.fano, extract(MONTH from exportacion.fnum) as mes,COUNT(DISTINCT departamento.nombre) as resultado FROM exportacion LEFT JOIN departamento ON ((((SUBSTRING(exportacion.ubigeo,1,2))= departamento.iddepartamento))) WHERE  substring(exportacion.ubigeo,1,2) = '".$codeid."' $condi1
GROUP BY exportacion.fano,mes order BY exportacion.fano, resultado ASC";
  }
  if($onevar=="provi"){
  $query1 = "SELECT exportacion.fano , extract(MONTH from exportacion.fnum) as mes, count(DISTINCT provincia.nombre) as resultado FROM exportacion LEFT JOIN departamento ON  (((SUBSTRING(exportacion.ubigeo,1,2))= departamento.iddepartamento))  INNER JOIN provincia ON provincia.iddepartamento = departamento.iddepartamento  WHERE  substring(exportacion.ubigeo,1,2) =  '".$codeid."' $condi1 GROUP BY exportacion.fano,mes ORDER BY exportacion.fano ASC";
  }
  
  if($onevar=="distri"){
  $query1 = "SELECT exportacion.fano, extract(MONTH from exportacion.fnum) as mes,COUNT(DISTINCT distrito.nombre) as resultado FROM exportacion LEFT JOIN departamento ON ((((SUBSTRING(exportacion.ubigeo,1,2))= departamento.iddepartamento))) INNER JOIN provincia ON provincia.iddepartamento = departamento.iddepartamento  INNER JOIN distrito ON distrito.idprovincia = provincia.idprovincia WHERE substring(exportacion.ubigeo,1,2) = '".$codeid."' $condi1 GROUP BY exportacion.fano,mes ORDER BY exportacion.fano ASC ";
  }
   if($onevar=="cage"){
  $query1 = "SELECT exportacion.fano, extract(MONTH from exportacion.fnum) as mes,COUNT(DISTINCT exportacion.cage) as resultado FROM exportacion WHERE  substring(exportacion.ubigeo,1,2) = '".$codeid."' $condi1 GROUP BY exportacion.fano,mes ORDER BY exportacion.fano,mes";
  }
  if($onevar=="cviatra"){
  $query1 = "SELECT exportacion.fano, extract(MONTH from exportacion.fnum) as mes,COUNT(DISTINCT exportacion.cviatra) as resultado FROM exportacion WHERE substring(exportacion.ubigeo,1,2) = '".$codeid."' $condi1 GROUP BY exportacion.fano,mes ORDER BY exportacion.fano,mes";
  }
...
if($resultado1!=0){//inicia si hay resultados
	...
	if($onevar=="diferen"){//precio general total
	  /*sumamos valorfob*/
		   ...
		   FROM exportacion WHERE $condiA1 AND substring(exportacion.ubigeo,1,2) ='".$codeid."'";
...
while($fila_vfob=$resultado_vfob->fetch_array()){
	...
	$vfob_2021= "0.00";
	  ...
	  /* sumamos vpesnet*/
	   ...
	   FROM $condiA1 AND substring(exportacion.ubigeo,1,2) ='".$codeid."'";
...
while($fila_vpes=$resultado_vpes->fetch_array()){
	...
	$vpes_2021= "0.00";
	  ...
	  $query_tota = "SELECT Count(DISTINCT exportacion.part_nandi) as cant_total, extract(year from exportacion.fnum) as anio FROM exportacion WHERE $condiA1 AND substring(exportacion.ubigeo,1,2) ='".$codeid."' GROUP BY anio order BY anio ASC";
	   $resultado_tota=$conexpg->query($query_tota); 
	    }//fin consulta conteo unico total partida
	 if($onevar=="ndcl"){//duas
	$query_tota = "SELECT Count(DISTINCT exportacion.ndcl) as cant_total, extract(year from exportacion.fnum) as anio FROM exportacion WHERE $condiA1 AND substring(exportacion.ubigeo,1,2) ='".$codeid."' GROUP BY anio order BY anio ASC";
	$resultado_tota=$conexpg->query($query_tota);
	 }
	 if($onevar=="dnombre"){//empresas
	 $query_tota = "SELECT Count(DISTINCT exportacion.dnombre) as cant_total, extract(year from exportacion.fnum) as anio FROM exportacion WHERE $condiA1 AND substring(exportacion.ubigeo,1,2) ='".$codeid."' GROUP BY anio order BY anio ASC";
	  $resultado_tota=$conexpg->query($query_tota);
	 }
	if($onevar=="cpaides"){//mercado
	  $query_tota = "SELECT Count(DISTINCT exportacion.cpaides) as cant_total, extract(year from exportacion.fnum) as anio FROM exportacion WHERE $condiA1 AND substring(exportacion.ubigeo,1,2)='".$codeid."' GROUP BY anio order BY anio ASC";
	  $resultado_tota=$conexpg->query($query_tota);
	 }
	 if($onevar=="cpuedes"){//puertos
	  $query_tota = "SELECT Count(DISTINCT exportacion.cpuedes) as cant_total, extract(year from exportacion.fnum) as anio FROM exportacion WHERE $condiA1 AND substring(exportacion.ubigeo,1,2) ='".$codeid."' GROUP BY anio order BY anio ASC";
	  $resultado_tota=$conexpg->query($query_tota);
	 }
	 if($onevar=="cadu"){//aduanas
	 $query_tota = "SELECT Count(DISTINCT exportacion.cadu) as cant_total, extract(year from exportacion.fnum) as anio FROM exportacion WHERE $condiA1 AND substring(exportacion.ubigeo,1,2) ='".$codeid."' GROUP BY anio order BY anio ASC";
	  $resultado_tota=$conexpg->query($query_tota);
	 }
	 if($onevar=="depa"){//departamento
	 $query_tota = "SELECT COUNT(DISTINCT departamento.nombre) as cant_total, extract(year from exportacion.fnum) as anio FROM exportacion LEFT JOIN departamento ON  ((((SUBSTRING(exportacion.ubigeo,1,2))= departamento.iddepartamento))) WHERE $condiA1 AND substring(exportacion.ubigeo,1,2)='".$codeid."' GROUP BY anio ORDER BY anio ASC";
	  $resultado_tota=$conexpg->query($query_tota);
	 }
	 if($onevar=="provi"){//provincia
     $query_tota = "SELECT count(DISTINCT provincia.nombre) as cant_total, extract(year from exportacion.fnum) as anio FROM exportacion LEFT JOIN departamento ON  (((SUBSTRING(exportacion.ubigeo,1,2))= departamento.iddepartamento))  INNER JOIN provincia ON provincia.iddepartamento = departamento.iddepartamento WHERE $condiA1 AND substring(exportacion.ubigeo,1,2)='".$codeid."' GROUP BY anio ORDER BY anio ASC";
	  $resultado_tota=$conexpg->query($query_tota);
	 }
	 if($onevar=="distri"){//distrito
	  $query_tota = "SELECT count(DISTINCT distrito.nombre) AS cant_total, extract(year from exportacion.fnum) AS anio FROM exportacion LEFT JOIN distrito ON exportacion.ubigeo= distrito.iddistrito WHERE $condiA1 AND substring(exportacion.ubigeo,1,2) = '".$codeid."' GROUP BY anio ORDER BY anio ASC";
	  $resultado_tota=$conexpg->query($query_tota);
	 }
	 if($onevar=="cage"){//agentes
	 $query_tota = "SELECT Count(DISTINCT exportacion.cage) as cant_total, extract(year from exportacion.fnum) as anio FROM exportacion WHERE $condiA1 AND substring(exportacion.ubigeo,1,2) ='".$codeid."' GROUP BY anio order BY anio ASC";
	  $resultado_tota=$conexpg->query($query_tota);
	 }
	 if($onevar=="cviatra"){//via transportes
	  $query_tota = "SELECT Count(DISTINCT exportacion.cviatra) as cant_total, extract(year from exportacion.fnum) as anio FROM exportacion WHERE $condiA1 AND substring(exportacion.ubigeo,1,2) ='".$codeid."' GROUP BY anio order BY anio ASC";
	  $resultado_tota=$conexpg->query($query_tota);
	 }
	 if($onevar=="part_nandi" or $onevar=="ndcl" or $onevar=="dnombre" or $onevar=="cpaides" or $onevar=="cpuedes" or $onevar=="cadu" or $onevar=="depa" or $onevar=="provi" or $onevar=="distri" or $onevar=="cage" or $onevar=="cviatra"){ 
	  ...
	  if($numReg_tota>0){
		   while($fila_tota=$numReg_tota->fetch_array()){
...
while($fila_year=$result_year->fetch_array()){
		...
		$sumatotal2019 = $vfob_2019/ $vpes_2019;
		}
				   
				   if($vpes_2020=='0'){
		$sumatotal2020 = "0.00";
		}else{
		$sumatotal2020 = $vfob_2020/ $vpes_2020;
		}
				   
				   if($vpes_2021=='0'){
		$sumatotal2021 = "0.00";
		}else{
		$sumatotal2021 = $vfob_2021/ $vpes_2021;
		}
		...
		if($sumatotal2012=='0'){
			  ...
			  $parti= number_format(($ca1_1 + $ca2_1 + $ca3_1 + $ca4_1 + $ca5_1 + $ca6_1 + $ca7_1 + $ca8_1)/8,2) ;//divide entre 5;porque solo suma 5 años del 2012 al 2016
			   }else if($onevar=="part_nandi"){
			 ...
			 if($sumatotal2018=='0' or $sumatotal2018 == null){
			  ...
			  $varitota= number_format((($sumatotal2020 / $sumatotal2019) - 1) * 100,2);
		      $parti= number_format(($parti_1 + $parti_2 + $parti_3 + $parti_4 + $parti_5 + $parti_6 + $parti_7 + $parti_8)/8,2) ;
			  ...
			  $varitota= number_format((($sumatotal2020 / $sumatotal2020) - 1) * 100,2);
			  $parti= number_format(($ca1_5 + $ca2_5 + $ca3_5 + $ca4_5 + $ca5_5 + $ca6_5 + $ca7_5 + $ca8_5)/8,2) ;
			  ...
			  $varitota= number_format((($sumatotal2020 / $sumatotal2019) - 1) * 100,2);
			  $parti= number_format(($ca1_6 + $ca2_6 + $ca3_6 + $ca4_6 + $ca5_6 + $ca6_6 + $ca7_6 + $ca8_6)/8,2) ;
			 ...
			 if($sumatotal2018=='0' or $sumatotal2018 == null){
			  ...
			  $varitota= number_format((($sumatotal2020 / $sumatotal2019) - 1) * 100,2);
			  $parti= number_format(($ca1_8 + $ca2_8 + $ca3_8 + $ca4_8 + $ca5_8 + $ca6_8 + $ca7_8 + $ca8_8)/8,2) ;
			 ...
			 if($sumatotal2018=='0' or $sumatotal2018 == null){
			  ...
			  $varitota= number_format((($sumatotal2020 / $sumatotal2019) - 1) * 100,2);
			  $parti= number_format(($ca1_9 + $ca2_9 + $ca3_9 + $ca4_9 + $ca5_9 + $ca6_9 + $ca7_9 + $ca8_9)/8,2) ;
		      ...
		      $ca7_9x = (($sumatotal2020 / $sumatotal2019) - 1) * 100;
		      }
			 
			  $varitota= number_format((($sumatotal2020 / $sumatotal2019) - 1) * 100,2);
			  $parti= number_format(($ca1_9x + $ca2_9x + $ca3_9x + $ca4_9x + $ca5_9x + $ca6_9x + $ca7_9x)/6,2) ;
		...
		$varitota= number_format((($sumatotal2020 / $sumatotal2019) - 1) * 100,2);
	    $parti= number_format(($ca1_dis + $ca2_dis + $ca3_dis + $ca4_dis + $ca5_dis + $ca6_dis + $ca7_dis + $ca8_dis)/8,2) ;
			 ...
			 if($sumatotal2018=='0' or $sumatotal2018 == null){
			  ...
			  $varitota= number_format((($sumatotal2020 / $sumatotal2019) - 1) * 100,2);
			  $parti= number_format(($ca1_10 + $ca2_10 + $ca3_10 + $ca4_10 + $ca5_10 + $ca6_10 + $ca7_10 + $ca8_10)/8,2) ;
			 ...
			 if($sumatotal2018=='0' or $sumatotal2018 == null){
			  ...
			  $varitota= number_format((($sumatotal2020 / $sumatotal2019) - 1) * 100,2);
			  $parti= number_format(($ca1_11 + $ca2_11 + $ca3_11 + $ca4_11 + $ca5_11 + $ca6_11 + $ca7_11 + $ca8_11)/8,2) ;
			   }else{
...
echo"<div class='col-md-12'>";
	...
	echo"<div class='card-header card-header-info card-header-icon'>
              ...
              <h4 class='card-title'><b>Reporte Ubigeo Evolucion Anual de Exportaciones</b><br> Ubigeo: $namedato │ Variable: $combo │ Fecha Numeracion │ Periodo 2012 - 2021 </b> </h4>
            </div>";
...
while($fila_rptx=$resultado_rptx->fetch_array()){
	...
	$year12x= $fila_rptx['a2020'];
		  ?>
		  ['<?php echo $nombredescx2; ?>',  <?php echo $year3x; ?>, <?php echo $year4x; ?>, <?php echo $year4x; ?>, <?php echo $year6x; ?>, <?php echo $year7x; ?>, <?php echo $year8x; ?>, <?php echo $year9x; ?>, <?php echo $year10x; ?>, <?php echo $year11x; ?>, <?php echo $year12x; ?>],
		  <?php
		  }
	  }
		  ?>
        ...
        var options = {
          title: 'Ubigeo <?php echo $namedato ?> Evolucion Anual de Exportaciones / <?php echo $combo ?> / 2012 - 2021',
          /*curveType: 'function',
