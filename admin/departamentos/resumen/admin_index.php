...
exportacion
GROUP BY
ax2012, ax2013, ax2014, ax2015, ax2016, ax2017, ax2018";
...
while($fila_depa19=$res_depa2019->fetch_array()){
  ...
  //$query1 = "SELECT substring(exportacion.ubigeo,1,2) as ubigeo FROM exportacion GROUP BY substring(exportacion.ubigeo,1,2) ORDER BY substring(exportacion.ubigeo,1,2) ASC";
  if($nombres2=="vfobserdol"){
  $query1 = "SELECT
...
exportacion AS e1
GROUP BY
substring(e1.ubigeo,1,2)
ORDER BY
substring(e1.ubigeo,1,2) ASC";
  }
		 ...
		 echo"<tfoot>
		  ...
		  //while ($fila1=pg_fetch_array($resultado1)) {
           ...
           $cod_ubigeo = $fila1['ubigeo'];
			  ...
			  /*$res_ubi = pg_query($query_ubi) or die("Error en la Consulta SQL UBIGEO");
	  $numReg_ubi = pg_num_rows($res_ubi);
	  if($numReg_ubi>0){
		  while ($fila_ubi=pg_fetch_array($res_ubi)) {*/
