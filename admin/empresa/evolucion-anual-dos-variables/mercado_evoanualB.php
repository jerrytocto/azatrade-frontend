...
FROM exportacion
LEFT JOIN paises ON exportacion.cpaides = paises.idpaises
WHERE exportacion.ndoc = '".$paiscode."' $querytu AND ".$wherefecha." GROUP BY exportacion.cpaides,paises.espanol ORDER BY paises.espanol ASC";
...
echo "<td>$var11%</td>";
 echo "</tr>";
	...
	echo"<h3 style='color:#fff;'><center><b>No hay datos</b></center></h3>";
}
		  ?>
...
FROM exportacion
LEFT JOIN paises ON exportacion.cpaides = paises.idpaises
WHERE exportacion.ndoc = '".$paiscode."' $querytu AND ".$wherefecha." GROUP BY exportacion.cpaides,paises.descripcion";
  }else if($nombres2=="total"){
	  $query_resultado = "SELECT
exportacion.cpaides,
paises.descripcion,
...
Sum(CASE ".$filtrofecha." WHEN '2021' THEN 1 ELSE 0 END) AS a2021
FROM exportacion
LEFT JOIN paises ON exportacion.cpaides = paises.idpaises WHERE exportacion.ndoc = '".$paiscode."' $querytu AND  ".$wherefecha." GROUP BY exportacion.cpaides,paises.descripcion";
  } else if($nombres2=="part_nandi" or $nombres2=="ndcl" or $nombres2=="dnombre" or $nombres2=="cpaides" or $nombres2=="cpuedes" or  $nombres2=="cadu" or $nombres2=="cage" or $nombres2=="cviatra"){
	  $query_resultado = "SELECT
exportacion.cpaides,
paises.descripcion,
...
while($fila_rpt=$resultado_rpt->fetch_array()){
		  ...
		  $year9= $fila_rpt['a2020'];
		   }
		  ?>
          ['<?php echo $nombredesc ?>', <?php echo $year10 ?>],
		  <?php
		  }
	  }
		  ?>
   ...
   <center>
        <h3 class="title" id="emp"><font color="#FFFFFF"> Empresa: <?=$paisname;?> │ Departamento: <?=$namedepa1;?> │ Evolucion Anual de Exportaciones │ <?=$combo;?> │ <?=$combo2;?> │ Periodo 2020 </font></h3></center>
