...
FROM exportacion
LEFT JOIN puerto ON exportacion.cpuedes = puerto.idcodigo WHERE exportacion.ndoc = '".$paiscode."' $querytu AND ".$wherefecha." ";
  }
  if($nombres2=="diferen"){
	  $sqlyear="SELECT
...
FROM exportacion
LEFT JOIN puerto ON exportacion.cpuedes = puerto.idcodigo
WHERE exportacion.ndoc = '".$paiscode."' $querytu AND ".$wherefecha." GROUP BY exportacion.cpuedes,puerto.puerto ";
...
".$wherefecha." GROUP BY exportacion.cpuedes,puerto.puerto ";
...
echo"<div class='col-md-12'>";
	...
	echo"<div class='card-header card-header-info card-header-icon'>
              ...
              <h4 class='card-title'><b>Reporte Empresa Evoluci&oacute;n Mensual de Exportaciones</b><br> Empresa: $paisname │ Ruc: $paiscode │ Departamento: $namedepa1 │ Indicador #1: $combo - Variable #2: $combo2 │ Fecha Numeracion │ Periodo 2012 - 2021 </b> </h4>
            </div>";
...
FROM exportacion
LEFT JOIN puerto ON exportacion.cpuedes = puerto.idcodigo
WHERE exportacion.ndoc = '".$paiscode."' '".$querytu."' AND ".$wherefecha." GROUP BY exportacion.cpuedes,puerto.puerto ORDER BY xx2020 DESC LIMIT 5";
  }else if($nombres2=="total"){
	  $query_resultado = "SELECT
exportacion.cpuedes,
puerto.puerto,
...
LEFT JOIN puerto ON exportacion.cpuedes = puerto.idcodigo
WHERE exportacion.ndoc = '".$paiscode."' '".$querytu."' AND
".$wherefecha." GROUP BY exportacion.cpuedes,puerto.puerto ORDER BY a2020 DESC LIMIT 5";
  } else if($nombres2=="part_nandi" or $nombres2=="ndcl" or $nombres2=="dnombre" or $nombres2=="cpaides" or $nombres2=="cpuedes" or  $nombres2=="cadu" or $nombres2=="cage" or $nombres2=="cviatra"){
	  $query_resultado = "SELECT
exportacion.cpuedes,
puerto.puerto,
...
exportacion
LEFT JOIN puerto ON exportacion.cpuedes = puerto.idcodigo
WHERE exportacion.ndoc = '".$paiscode."' '".$querytu."' AND ".$wherefecha." GROUP BY exportacion.cpuedes,puerto.puerto ORDER BY a2020 DESC LIMIT 5";
  }else{
	   if($nombres2=="depa"){
		  $variaA='substring(exportacion.ubigeo,1,2)';
	  }else if($nombres2=="provi"){
...
?>
      ...
      var options = {
        title: 'Empresa: <?php echo $paisname ?> - Departamento: <?=$namedepa1;?> - Evolución Mensual <?php echo $combo ?> <?php echo $combo2 ?> de Exportaciones Periodo 2012 - 2021',
        chartArea: {width: '50%'},
        isStacked: true,
        hAxis: {
