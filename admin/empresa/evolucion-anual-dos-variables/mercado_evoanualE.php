...
FROM exportacion
LEFT JOIN aduana ON exportacion.cadu = aduana.codadu WHERE exportacion.ndoc = '".$paiscode."' $querytu AND ".$wherefecha." ";
  }
  if($nombres2=="diferen"){
	  $sqlyear="SELECT
...
LEFT JOIN aduana ON exportacion.cadu = aduana.codadu
WHERE
exportacion.ndoc = '".$paiscode."' $querytu AND ".$wherefecha." ";
...
FROM exportacion
LEFT JOIN aduana ON exportacion.cadu = aduana.codadu
WHERE exportacion.ndoc = '".$paiscode."' $querytu AND ".$wherefecha." GROUP BY exportacion.cadu,aduana.descripcion ";
...
".$wherefecha." GROUP BY exportacion.cadu,aduana.descripcion ";
...
echo"<div class='col-md-12'>";
	...
	echo"<div class='card-header card-header-info card-header-icon'>
              ...
              <h4 class='card-title'><b>Reporte Empresa Evoluci&oacute;n Anual de Exportaciones</b><br> Empresa: $paisname │ Ruc: $paiscode │ Departamento: $namedepa1 │ Indicador #1: $combo - Variable #2: $combo2 │ Fecha Numeracion │ Periodo 2012 - 2021 </b> </h4>
            </div>";
...
FROM exportacion
LEFT JOIN aduana ON exportacion.cadu = aduana.codadu
WHERE exportacion.ndoc = '".$paiscode."' $querytu AND ".$wherefecha." GROUP BY exportacion.cadu,aduana.descripcion ORDER BY xx2020 DESC LIMIT 5";
  }else if($nombres2=="total"){
	  $query_resultado = "SELECT
exportacion.cadu,
aduana.descripcion,
...
FROM exportacion
LEFT JOIN aduana ON exportacion.cadu = aduana.codadu
WHERE exportacion.cpaides = '".$paiscode."' $querytu AND
".$wherefecha." GROUP BY exportacion.cadu,aduana.descripcion ORDER BY a2020 DESC LIMIT 5";
  } else if($nombres2=="part_nandi" or $nombres2=="ndcl" or $nombres2=="dnombre" or $nombres2=="cpaides" or $nombres2=="cpuedes" or  $nombres2=="cadu" or $nombres2=="cage" or $nombres2=="cviatra"){
	  $query_resultado = "SELECT
exportacion.cadu,
aduana.descripcion,
...
exportacion
LEFT JOIN aduana ON exportacion.cadu = aduana.codadu
WHERE exportacion.ndoc = '".$paiscode."' $querytu AND ".$wherefecha." GROUP BY exportacion.cadu,aduana.descripcion ORDER BY a2020 DESC LIMIT 5";
  }else{
	  if($nombres2=="depa"){
		  $variaA='substring(exportacion.ubigeo,1,2)';
	  }else if($nombres2=="provi"){
...
?>
      ...
      var options = {
        title: 'Evolución Anual Empresa: <?=$paisname;?> - Departamento: <?=$namedepa1;?> - <?php echo $combo ?> <?php echo $combo2 ?> de Exportaciones',
        chartArea: {width: '50%'},
        isStacked: true,
        hAxis: {
