...
while($fila_year=$result_year->fetch_array()){
		   ...
		   $varitota= (($sumatotal2019 / $sumatotal2018) - 1) * 100;
		   }
		   $parti='100';
		   }
	  }else{
	...
	while($rwpa1=$rsparti->fetch_array()){
	 ...
	 $nom_parti="-----";
	      ...
	      if($nombres2=="diferen"){
			  ...
			  if($dife9B=='0'){
				   $year9= "0.00";  
			   }else{ $year9= number_format($dife9A / $dife9B,2); }
		...
		if($year7=='0'){
		  ...
		  $var11 = (($year8 / $year7) - 1) * 100;
		  }
		  
		if($sumatotal2019=="0"){
			  $var22 ="0.00";
		  }else{
		  $var22 = ($year8 / $sumatotal2019) * 100;
		  }
...
echo "<td>$var22%</td>";
 echo "</tr>";*/
				  ...
				  //insertamos detalle
				$Sql_inserparti="insert into resumen_partidas (varia_filtro, codigo, descripcion, anio1, anio2, anio3, anio4, anio5, anio6, anio7, anio8, anio9, variauno, variados
)  values (
'$nombres2',
'$cod_part',
'$nom_parti',
