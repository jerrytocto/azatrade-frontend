...
FROM exportacion
LEFT JOIN departamento ON substring(exportacion.ubigeo,1,2) = departamento.iddepartamento WHERE exportacion.cpaides = '".$paiscode."' AND ".$wherefecha." $queryu ";
  }
  if($nombres2=="diferen"){
	  $sqlyear="SELECT
...
FROM exportacion
LEFT JOIN departamento ON substring(exportacion.ubigeo,1,2) = departamento.iddepartamento
WHERE exportacion.cpaides = '".$paiscode."' AND ".$wherefecha." $queryu GROUP BY departamento.iddepartamento,departamento.nombre ";
...
".$wherefecha." $queryu GROUP BY departamento.iddepartamento,departamento.nombre ";
...
<?php
  ...
  <center>
        <h3 class="title" id="mapa"><font color="#FFFFFF">Mapa Geografico │ Mercado: <?=$paisname;?> │ Departamento: <?=$namedepa1;?> │ <?=$combo;?> │ Periodo 2020 │ Ubigeo Anual de Exportaciones</font></h3></center>
