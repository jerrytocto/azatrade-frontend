...
FROM exportacion
LEFT JOIN aduana ON exportacion.cadu = aduana.codadu WHERE exportacion.cpaides = '".$paiscode."' AND ".$wherefecha." AND SUBSTRING(exportacion.ubigeo,1,2) like '%$flatcod' ";
  }
  if($nombres2=="diferen"){
	  $sqlyear="SELECT
...
FROM exportacion
LEFT JOIN aduana ON exportacion.cadu = aduana.codadu
WHERE exportacion.cpaides = '".$paiscode."' AND ".$wherefecha." AND SUBSTRING(exportacion.ubigeo,1,2) like '%$flatcod' GROUP BY exportacion.cadu,aduana.descripcion ";
...
".$wherefecha." AND SUBSTRING(exportacion.ubigeo,1,2) like '%$flatcod' GROUP BY exportacion.cadu,aduana.descripcion ";
...
LEFT JOIN aduana ON exportacion.cadu = aduana.codadu
WHERE exportacion.cpaides = '".$paiscode."' AND ".$wherefecha." AND SUBSTRING(exportacion.ubigeo,1,2) like '%$flatcod'
GROUP BY exportacion.cadu,aduana.descripcion ORDER BY a2018 DESC LIMIT 5";
  }else if ($nombres2=="diferen"){
	  $query_resultado = "SELECT
exportacion.cadu,
aduana.descripcion,
...
FROM exportacion
LEFT JOIN aduana ON exportacion.cadu = aduana.codadu
WHERE exportacion.cpaides = '".$paiscode."' AND ".$wherefecha." AND SUBSTRING(exportacion.ubigeo,1,2) like '%$flatcod' GROUP BY exportacion.cadu,aduana.descripcion ORDER BY xx2018 DESC LIMIT 5";
  }else if($nombres2=="total"){
	  $query_resultado = "SELECT
exportacion.cadu,
aduana.descripcion,
...
LEFT JOIN aduana ON exportacion.cadu = aduana.codadu
WHERE exportacion.cpaides = '".$paiscode."' AND
".$wherefecha." AND SUBSTRING(exportacion.ubigeo,1,2) like '%$flatcod' GROUP BY exportacion.cadu,aduana.descripcion ORDER BY a2018 DESC LIMIT 5";
  } else if($nombres2=="part_nandi" or $nombres2=="ndcl" or $nombres2=="dnombre" or $nombres2=="cpaides" or $nombres2=="cpuedes" or  $nombres2=="cadu" or $nombres2=="cage" or $nombres2=="cviatra"){
	  $query_resultado = "SELECT
exportacion.cadu,
aduana.descripcion,
...
exportacion
LEFT JOIN aduana ON exportacion.cadu = aduana.codadu
WHERE exportacion.cpaides = '".$paiscode."' AND ".$wherefecha." AND SUBSTRING(exportacion.ubigeo,1,2) like '%$flatcod' GROUP BY exportacion.cadu,aduana.descripcion ORDER BY a2018 DESC LIMIT 5";
  }else{
	  if($nombres2=="depa"){
		  $variaA='substring(exportacion.ubigeo,1,2)';
	  }else if($nombres2=="provi"){
...
?>
      ...
      var options = {
        title: 'Evolución Anual Mercado: <?=$paisname;?> Departamento: <?=$namedepa1;?>  <?php echo $combo ?> <?php echo $combo2 ?> de Exportaciones - Periodo 2012 - 2019',
        chartArea: {width: '50%'},
        isStacked: true,
        hAxis: {
