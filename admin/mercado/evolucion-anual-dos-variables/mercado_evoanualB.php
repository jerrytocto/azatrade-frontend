...
FROM exportacion
WHERE exportacion.cpaides = '".$paiscode."' AND ".$wherefecha." $queryu GROUP BY exportacion.dnombre ORDER BY exportacion.dnombre ASC";
...
".$wherefecha." $queryu GROUP BY exportacion.dnombre ORDER BY exportacion.dnombre ASC";
...
echo"<div class='col-md-12'>";
	...
	echo"<div class='card-header card-header-info card-header-icon'>
              ...
              <h4 class='card-title'><b>Reporte Mercado Evoluci&oacute;n Mensual de Exportaciones</b><br> Mercado: $paisname │ Departamento: $namedepa1 │ Variable #1: $combo - Variable #2: $combo2 │ Fecha Numeracion │ Periodo 2012 - 2021 </b> </h4>
            </div>";
...
FROM exportacion
WHERE exportacion.cpaides = '".$paiscode."' AND ".$wherefecha." ".$queryu." 
GROUP BY exportacion.dnombre ORDER BY a2020 DESC LIMIT 5";
  }else if ($nombres2=="diferen"){
	  $query_resultado = "SELECT
exportacion.dnombre,
Sum(CASE ".$filtrofecha." WHEN '2012' THEN exportacion.vfobserdol ELSE 0 END) AS x2012,
...
Sum(CASE ".$filtrofecha." WHEN '2021' THEN exportacion.vpesnet ELSE 0 END) AS xx2021
FROM exportacion
WHERE exportacion.cpaides = '".$paiscode."' AND ".$wherefecha." ".$queryu."  GROUP BY exportacion.dnombre ORDER BY xx2020 DESC LIMIT 5";
  }else if($nombres2=="total"){
	  $query_resultado = "SELECT
exportacion.dnombre,
Sum(CASE ".$filtrofecha." WHEN '2012' THEN 1 ELSE 0 END) AS a2012,
...
FROM exportacion
WHERE exportacion.cpaides = '".$paiscode."' AND
".$wherefecha." ".$queryu." GROUP BY exportacion.dnombre ORDER BY a2020 DESC LIMIT 5";
  } else if($nombres2=="part_nandi" or $nombres2=="ndcl" or $nombres2=="dnombre" or $nombres2=="cpaides" or $nombres2=="cpuedes" or  $nombres2=="cadu" or $nombres2=="cage" or $nombres2=="cviatra"){
	  $query_resultado = "SELECT
exportacion.dnombre,
COUNT(distinct CASE ".$filtrofecha." WHEN '2012' THEN  exportacion.".$nombres2." ELSE NULL END) AS a2012,
...
FROM
exportacion
WHERE exportacion.cpaides = '".$paiscode."' AND ".$wherefecha." ".$queryu." GROUP BY exportacion.dnombre ORDER BY a2020 DESC LIMIT 5";
  }else{
	  if($nombres2=="depa"){
		  $variaA='substring(exportacion.ubigeo,1,2)';
	  }else if($nombres2=="provi"){
