...
FROM exportacion
LEFT JOIN aduana ON exportacion.cadu = aduana.codadu WHERE exportacion.cpaides = '".$paiscode."' AND ".$wherefecha." $queryu ";
  }
  if($nombres2=="diferen"){
	  $sqlyear="SELECT
...
FROM exportacion
LEFT JOIN aduana ON exportacion.cadu = aduana.codadu
WHERE exportacion.cpaides = '".$paiscode."' AND ".$wherefecha." $queryu ";
  }
...
LEFT JOIN aduana ON exportacion.cadu = aduana.codadu
WHERE
exportacion.cpaides = '".$paiscode."' AND ".$wherefecha." $queryu ";
...
LEFT JOIN aduana ON exportacion.cadu = aduana.codadu
WHERE exportacion.cpaides = '".$paiscode."' AND ".$wherefecha." $queryu 
GROUP BY exportacion.cadu,aduana.descripcion ";
...
FROM exportacion
LEFT JOIN aduana ON exportacion.cadu = aduana.codadu
WHERE exportacion.cpaides = '".$paiscode."' AND ".$wherefecha." $queryu GROUP BY exportacion.cadu,aduana.descripcion ";
...
".$wherefecha." $queryu GROUP BY exportacion.cadu,aduana.descripcion ";
...
echo"<div class='col-md-12'>";
	...
	echo"<div class='card-header card-header-info card-header-icon'>
              ...
              <h4 class='card-title'><b>Reporte Mercado Evoluci&oacute;n Mensual de Exportaciones</b><br> Mercado: $paisname │ Departamento: $namedepa1 │ Indicador #1: $combo - Variable #2: $combo2 │ Fecha Numeracion │ Periodo 2012 - 2021 </b> </h4>
            </div>";
...
Sum(CASE ".$filtrofecha." WHEN '2019' THEN ".$nombres2." ELSE 0 END) AS a2019,
Sum(CASE ".$filtrofecha." WHEN '2020' THEN ".$nombres2." ELSE 0 END) AS a2020,
Sum(CASE ".$filtrofecha." WHEN '2020' THEN ".$nombres2." ELSE 0 END) AS a2021
FROM exportacion
LEFT JOIN aduana ON exportacion.cadu = aduana.codadu
WHERE exportacion.cpaides = '".$paiscode."' AND ".$wherefecha." ".$queryu." 
GROUP BY exportacion.cadu,aduana.descripcion ORDER BY a2020 DESC LIMIT 5";
  }else if ($nombres2=="diferen"){
	  $query_resultado = "SELECT
exportacion.cadu,
aduana.descripcion,
...
FROM exportacion
LEFT JOIN aduana ON exportacion.cadu = aduana.codadu
WHERE exportacion.cpaides = '".$paiscode."' AND ".$wherefecha." ".$queryu."  GROUP BY exportacion.cadu,aduana.descripcion ORDER BY xx2020 DESC LIMIT 5";
  }else if($nombres2=="total"){
	  $query_resultado = "SELECT
exportacion.cadu,
aduana.descripcion,
...
LEFT JOIN aduana ON exportacion.cadu = aduana.codadu
WHERE exportacion.cpaides = '".$paiscode."' AND
".$wherefecha." ".$queryu." GROUP BY exportacion.cadu,aduana.descripcion ORDER BY a2020 DESC LIMIT 5";
  } else if($nombres2=="part_nandi" or $nombres2=="ndcl" or $nombres2=="dnombre" or $nombres2=="cpaides" or $nombres2=="cpuedes" or  $nombres2=="cadu" or $nombres2=="cage" or $nombres2=="cviatra"){
	  $query_resultado = "SELECT
exportacion.cadu,
aduana.descripcion,
...
exportacion
LEFT JOIN aduana ON exportacion.cadu = aduana.codadu
WHERE exportacion.cpaides = '".$paiscode."' AND ".$wherefecha." ".$queryu." GROUP BY exportacion.cadu,aduana.descripcion ORDER BY a2020 DESC LIMIT 5";
  }else{
	  if($nombres2=="depa"){
		  $variaA='substring(exportacion.ubigeo,1,2)';
	  }else if($nombres2=="provi"){
...
?>
      ...
      var options = {
        title: 'Evolución Anual Mercado: <?=$paisname;?> Departamento: <?=$namedepa1;?>  <?php echo $combo ?> <?php echo $combo2 ?> de Exportaciones - Periodo 2012 - 2021',
        chartArea: {width: '50%'},
        isStacked: true,
        hAxis: {
