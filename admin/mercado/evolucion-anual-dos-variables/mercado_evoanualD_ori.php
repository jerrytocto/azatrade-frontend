...
FROM exportacion
LEFT JOIN agente ON exportacion.cadu = agente.idagente 
...
FROM exportacion
LEFT JOIN agente ON exportacion.cage = agente.idagente 
WHERE exportacion.cpaides = '".$paiscode."' AND ".$wherefecha." AND SUBSTRING(exportacion.ubigeo,1,2) like '%$flatcod' ";
  }
...
FROM exportacion
WHERE exportacion.cpaides = '".$paiscode."' AND ".$wherefecha." AND SUBSTRING(exportacion.ubigeo,1,2) like '%$flatcod' GROUP BY exportacion.cage ";
...
".$wherefecha." AND SUBSTRING(exportacion.ubigeo,1,2) like '%$flatcod' GROUP BY exportacion.cage ";
...
echo"<div class='col-md-12'>";
	...
	echo"<div class='card-header card-header-info card-header-icon'>
              ...
              <h4 class='card-title'><b>Reporte Mercado Evoluci&oacute;n Anual de Exportaciones</b><br> Mercado: $paisname │ Departamento: $namedepa1 │ Variable #1: $combo - Variable #2: $combo2 │ Fecha Numeracion │ Periodo 2012 - 2019 </b> </h4>
            </div>";
 ...
 //while ($fila_rpt=pg_fetch_array($resultado_rpt)) {
	 ...
	 $cuentaD = $cuentaD + 1;
			  ...
			  /*$resadu = pg_query($query_aduana) or die("Error en la Consulta SQL Reporte");
	  $numReg_rptadu = pg_num_rows($resadu);
	  if($numReg_rptadu>0){
		   while ($fila_rptadu=pg_fetch_array($resadu)) {*/
		 $resadu=$conexpg->query($query_aduana); 
if($resadu->num_rows>0){ 
while($fila_rptadu=$resadu->fetch_array()){
...
FROM exportacion
WHERE exportacion.cpaides = '".$paiscode."' AND ".$wherefecha." AND SUBSTRING(exportacion.ubigeo,1,2) like '%$flatcod'
GROUP BY exportacion.cage ORDER BY a2018 DESC LIMIT 5 ";
  }else if ($nombres2=="diferen"){
	  $query_resultado = "SELECT
exportacion.cage,
Sum(CASE ".$filtrofecha." WHEN '2012' THEN exportacion.vfobserdol ELSE 0 END) AS x2012,
...
Sum(CASE ".$filtrofecha." WHEN '2019' THEN exportacion.vpesnet ELSE 0 END) AS xx2019
FROM exportacion
WHERE exportacion.cpaides = '".$paiscode."' AND ".$wherefecha." AND SUBSTRING(exportacion.ubigeo,1,2) like '%$flatcod' GROUP BY exportacion.cage ORDER BY xx2018 DESC LIMIT 5";
  }else if($nombres2=="total"){
	  $query_resultado = "SELECT
exportacion.cage,
Sum(CASE ".$filtrofecha." WHEN '2012' THEN 1 ELSE 0 END) AS a2012,
...
FROM exportacion
WHERE exportacion.cpaides = '".$paiscode."' AND
".$wherefecha." AND SUBSTRING(exportacion.ubigeo,1,2) like '%$flatcod' GROUP BY exportacion.cage ORDER BY a2018 DESC LIMIT 5";
  } else if($nombres2=="part_nandi" or $nombres2=="ndcl" or $nombres2=="dnombre" or $nombres2=="cpaides" or $nombres2=="cpuedes" or  $nombres2=="cadu" or $nombres2=="cage" or $nombres2=="cviatra"){
	  $query_resultado = "SELECT
exportacion.cage,
COUNT(distinct CASE ".$filtrofecha." WHEN '2012' THEN  exportacion.".$nombres2." ELSE NULL END) AS a2012,
...
COUNT(distinct CASE ".$filtrofecha." WHEN '2019' THEN  exportacion.".$nombres2." ELSE NULL END) AS a2019
FROM exportacion
WHERE exportacion.cpaides = '".$paiscode."' AND ".$wherefecha." AND SUBSTRING(exportacion.ubigeo,1,2) like '%$flatcod' GROUP BY exportacion.cage ORDER BY a2017 DESC LIMIT 5";
  }else{
	  if($nombres2=="depa"){
		  $variaA='substring(exportacion.ubigeo,1,2)';
	  }else if($nombres2=="provi"){
...
?>
      ...
      var options = {
        title: 'Evolución Anual Mercado: <?=$paisname;?>  <?php echo $combo ?> <?php echo $combo2 ?> de Exportaciones - Periodo 2012 - 2019',
        chartArea: {width: '50%'},
        isStacked: true,
        hAxis: {
