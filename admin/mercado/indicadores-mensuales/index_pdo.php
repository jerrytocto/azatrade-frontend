...
define('DB_USER','root');
define('DB_PASS','jopedis');
define('DB_NAME','azatradeexpo');*/

define('DB_HOST','localhost');
define('DB_USER','azabusin_trade');
define('DB_PASS','&SJqA5UErXKM');
define('DB_NAME','azabusin_azatrade');
...
set_time_limit(500);
$paisname = $_POST["namepaisB"];
$paiscode = $_POST["codepaisB"];
$anio = $_POST["years"];
...
/*sumamos valorfob*/
		   ...
		   FROM exportacion WHERE extract(year from exportacion.fnum)= '$anio' AND SUBSTRING(exportacion.ubigeo,1,2) = '$flatcod' AND exportacion.cpaides='$paiscode'";
				/*$resultado_vfob=$conexpg->query($query_vfob); 
if($resultado_vfob->num_rows>0){ 
while($fila_vfob=$resultado_vfob->fetch_array()){*/
				$query = $connect -> prepare($query_vfob); 
$query -> execute(); 
$results = $query -> fetchAll(PDO::FETCH_OBJ); 
if($query -> rowCount() > 0)   { 
foreach($results as $fila_vfob) { 
...
while($fila_vpes=$resultado_vpes->fetch_array()){*/
				$query2 = $connect -> prepare($query_vpes); 
$query2 -> execute(); 
$results2 = $query2 -> fetchAll(PDO::FETCH_OBJ); 
if($query2 -> rowCount() > 0)   { 
foreach($results2 as $fila_vpes) {
...
SUM(CASE extract(MONTH from exportacion.fnum) WHEN '12' THEN exportacion.vpesbru ELSE 0 END) AS diciembre FROM exportacion where extract(year from exportacion.fnum)= '$anio' AND SUBSTRING(exportacion.ubigeo,1,2) = '$flatcod' AND exportacion.cpaides='$paiscode' ";
				/*$resultado_vbru=$conexpg->query($query_vbru); 
if($resultado_vbru->num_rows>0){ 
while($fila_vbru=$resultado_vbru->fetch_array()){*/
				$query3 = $connect -> prepare($query_vbru); 
$query3 -> execute(); 
$results3 = $query3 -> fetchAll(PDO::FETCH_OBJ); 
if($query3 -> rowCount() > 0)   { 
foreach($results3 as $fila_vbru) {
...
while($fila_qunifis=$resultado_qunifis->fetch_array()){*/
				$query4 = $connect -> prepare($query_qunifis); 
$query4 -> execute(); 
$results4 = $query4 -> fetchAll(PDO::FETCH_OBJ); 
if($query4 -> rowCount() > 0)   { 
foreach($results4 as $fila_qunifis) {
...
while($fila_qunicom=$resultado_qunicom->fetch_array()){*/
				$query5 = $connect -> prepare($query_qunicom); 
$query5 -> execute(); 
$results5 = $query5 -> fetchAll(PDO::FETCH_OBJ); 
if($query5 -> rowCount() > 0)   { 
foreach($results5 as $fila_qunicom) {
	...
	$qunicom_12 = $fila_qunicom -> diciembre;
	  ...
	  $query_parti = "SELECT Count(DISTINCT exportacion.part_nandi) as cant_parti, extract(MONTH from exportacion.fnum) as mes FROM exportacion WHERE extract(year from exportacion.fnum)= '$anio' AND SUBSTRING(exportacion.ubigeo,1,2) = '$flatcod' AND exportacion.cpaides='$paiscode' GROUP BY mes order BY mes ASC";
				/*$resultado_parti=$conexpg->query($query_parti); 
if($resultado_parti->num_rows>0){ 
while($fila_parti=$resultado_parti->fetch_array()){*/
			$query6 = $connect -> prepare($query_parti); 
$query6 -> execute(); 
$results6 = $query6 -> fetchAll(PDO::FETCH_OBJ); 
if($query6 -> rowCount() > 0)   { 
foreach($results6 as $fila_parti) {
	$anioparti = $fila_parti -> mes;
	  ...
	  $generalA = "SELECT Count(DISTINCT exportacion.part_nandi) as cant_parti FROM exportacion WHERE extract(year from exportacion.fnum)= '$anio' AND SUBSTRING(exportacion.ubigeo,1,2) = '$flatcod' AND exportacion.cpaides='$paiscode' ";
				/*$res_generaA=$conexpg->query($generalA); 
if($res_generaA->num_rows>0){ 
while($fila_generaA=$res_generaA->fetch_array()){*/
				$query7 = $connect -> prepare($generalA); 
$query7 -> execute(); 
$results7 = $query7 -> fetchAll(PDO::FETCH_OBJ); 
if($query7 -> rowCount() > 0)   { 
foreach($results7 as $fila_generaA) {
...
while($fila_creg=$resultado_creg->fetch_array()){*/
				$query8 = $connect -> prepare($query_creg); 
$query8 -> execute(); 
$results8 = $query8 -> fetchAll(PDO::FETCH_OBJ); 
if($query8 -> rowCount() > 0)   { 
foreach($results7 as $fila_creg) {
	...
	$creg_12 = $fila_creg -> diciembre;
	  ...
	  /*cuenta cantidad de duas*/
	   $query_dua = "SELECT Count(DISTINCT exportacion.ndcl) as cant_dua, extract(MONTH from exportacion.fnum) as mes FROM exportacion WHERE extract(year from exportacion.fnum)= '$anio' AND SUBSTRING(exportacion.ubigeo,1,2) = '$flatcod' AND exportacion.cpaides='$paiscode' GROUP BY mes order BY mes ASC";
				/*$resultado_dua=$conexpg->query($query_dua); 
if($resultado_dua->num_rows>0){ 
while($fila_dua=$resultado_dua->fetch_array()){*/
				$query9 = $connect -> prepare($query_dua); 
$query9 -> execute(); 
$results9 = $query9 -> fetchAll(PDO::FETCH_OBJ); 
if($query9 -> rowCount() > 0)   { 
foreach($results9 as $fila_dua) {
	...
	$aniodua = $fila_dua -> mes;
	  ...
	  $generalB = "SELECT Count(DISTINCT exportacion.ndcl) as cant_dua FROM exportacion WHERE extract(year from exportacion.fnum)= '$anio' AND SUBSTRING(exportacion.ubigeo,1,2) = '$flatcod' AND exportacion.cpaides='$paiscode' ";
				/*$res_generaB=$conexpg->query($generalB); 
if($res_generaB->num_rows>0){ 
while($fila_generaB=$res_generaB->fetch_array()){*/
				$query10 = $connect -> prepare($generalB); 
$query10 -> execute(); 
$results10 = $query10 -> fetchAll(PDO::FETCH_OBJ); 
if($query10 -> rowCount() > 0)   { 
foreach($results10 as $fila_generaB) {
	...
	$totadua = $fila_generaB -> cant_dua;
	  ...
	  /*cuenta cantidad de empresas*/
	   $query_emp = "SELECT Count(DISTINCT exportacion.dnombre) as cant_nom, extract(MONTH from exportacion.fnum) as mes FROM exportacion WHERE extract(year from exportacion.fnum)= '$anio' AND SUBSTRING(exportacion.ubigeo,1,2) = '$flatcod' AND exportacion.cpaides='$paiscode' GROUP BY mes order BY mes ASC";
				/*$resultado_emp=$conexpg->query($query_emp); 
if($resultado_emp->num_rows>0){ 
while($fila_emp=$resultado_emp->fetch_array()){*/
				$query11 = $connect -> prepare($query_emp); 
$query11 -> execute(); 
$results11 = $query11 -> fetchAll(PDO::FETCH_OBJ); 
if($query11 -> rowCount() > 0)   { 
foreach($results11 as $fila_emp) {
	...
	$anioemp = $fila_emp -> mes;
	  ...
	  $generalC = "SELECT Count(DISTINCT exportacion.dnombre) as cant_nom FROM exportacion WHERE extract(year from exportacion.fnum)= '$anio' AND SUBSTRING(exportacion.ubigeo,1,2) = '$flatcod' AND exportacion.cpaides='$paiscode' ";
				/*$res_generaC=$conexpg->query($generalC); 
if($res_generaC->num_rows>0){ 
while($fila_generaC=$res_generaC->fetch_array()){*/
				$query12 = $connect -> prepare($query_emp); 
$query12 -> execute(); 
$results12 = $query12 -> fetchAll(PDO::FETCH_OBJ); 
if($query12 -> rowCount() > 0)   { 
foreach($results12 as $fila_generaC) {
	...
	$totaempre = $fila_generaC -> cant_nom;
	 ...
	 /* $query_merca = "SELECT Count(DISTINCT exportacion.cpaides) as cant_mer, extract(MONTH from exportacion.fnum) as mes FROM exportacion WHERE extract(year from exportacion.fnum)= '$anio' AND exportacion.cpaides='$paiscode' GROUP BY mes order BY mes ASC";
	  ...
	  /*contamos cantidad de puertos*/
	   $query_pue = "SELECT Count(DISTINCT exportacion.cpuedes) as cant_pue, extract(MONTH from exportacion.fnum) as mes FROM exportacion WHERE extract(year from exportacion.fnum)= '$anio' AND SUBSTRING(exportacion.ubigeo,1,2) = '$flatcod' AND exportacion.cpaides='$paiscode' GROUP BY mes order BY mes ASC";
				/*$resultado_pue=$conexpg->query($query_pue); 
if($resultado_pue->num_rows>0){ 
while($fila_pue=$resultado_pue->fetch_array()){*/
				$query13 = $connect -> prepare($query_pue); 
$query13 -> execute(); 
$results13 = $query13 -> fetchAll(PDO::FETCH_OBJ); 
if($query13 -> rowCount() > 0)   { 
foreach($results13 as $fila_pue) {
	...
	$aniopue = $fila_pue -> mes;
	  ...
	  $generalD = "SELECT Count(DISTINCT exportacion.cpuedes) as cant_pue FROM exportacion WHERE extract(year from exportacion.fnum)= '$anio' AND SUBSTRING(exportacion.ubigeo,1,2) = '$flatcod' AND exportacion.cpaides='$paiscode' ";
				/*$res_generaD=$conexpg->query($generalD); 
if($res_generaD->num_rows>0){ 
while($fila_generaD=$res_generaD->fetch_array()){*/
				$query14 = $connect -> prepare($generalD); 
$query14 -> execute(); 
$results14 = $query14 -> fetchAll(PDO::FETCH_OBJ); 
if($query14 -> rowCount() > 0)   { 
foreach($results14 as $fila_generaD) {
	...
	$totapuerto = $fila_generaD -> cant_pue;
	  ...
	  /*contamos cantidad de aduanas*/
	   $query_adua = "SELECT Count(DISTINCT exportacion.cadu) as cant_adua, extract(MONTH from exportacion.fnum) as mes FROM exportacion WHERE extract(year from exportacion.fnum)= '$anio' AND SUBSTRING(exportacion.ubigeo,1,2) = '$flatcod' AND exportacion.cpaides='$paiscode' GROUP BY mes order BY mes ASC";
				/*$resultado_adua=$conexpg->query($query_adua); 
if($resultado_adua->num_rows>0){ 
while($fila_adua=$resultado_adua->fetch_array()){*/
				$query15 = $connect -> prepare($query_adua); 
$query15 -> execute(); 
$results15 = $query15 -> fetchAll(PDO::FETCH_OBJ); 
if($query15 -> rowCount() > 0)   { 
foreach($results15 as $fila_adua) {
	  ...
	  $generalE = "SELECT Count(DISTINCT exportacion.cadu) as cant_adua FROM exportacion WHERE extract(year from exportacion.fnum)= '$anio' AND SUBSTRING(exportacion.ubigeo,1,2) = '$flatcod' AND exportacion.cpaides='$paiscode' ";
				/*$res_generaE=$conexpg->query($generalE); 
if($res_generaE->num_rows>0){ 
while($fila_generaE=$res_generaE->fetch_array()){*/
				$query16 = $connect -> prepare($generalE); 
$query16 -> execute(); 
$results16 = $query16 -> fetchAll(PDO::FETCH_OBJ); 
if($query16 -> rowCount() > 0)   { 
foreach($results16 as $fila_generaE) {
...
foreach($results17 as $fila_merca) {
	  ...
	  $generalF = "SELECT COUNT(DISTINCT departamento.nombre) as departamento FROM exportacion LEFT JOIN departamento ON  ((((SUBSTRING(exportacion.ubigeo,1,2))= departamento.iddepartamento))) WHERE extract(year from exportacion.fnum)= '$anio' AND SUBSTRING(exportacion.ubigeo,1,2) = '$flatcod' AND exportacion.cpaides='$paiscode' ";
				/*$res_generaF=$conexpg->query($generalF); 
if($res_generaF->num_rows>0){ 
while($fila_generaF=$res_generaF->fetch_array()){*/
				$query18 = $connect -> prepare($generalF); 
$query18 -> execute(); 
$results18 = $query18 -> fetchAll(PDO::FETCH_OBJ); 
if($query18 -> rowCount() > 0)   { 
foreach($results18 as $fila_generaF) {
...
while($fila_prov=$resultado_prov->fetch_array()){*/
				$query19 = $connect -> prepare($query_prov); 
$query19 -> execute(); 
$results19 = $query19 -> fetchAll(PDO::FETCH_OBJ); 
if($query19 -> rowCount() > 0)   { 
foreach($results19 as $fila_prov) {
...
foreach($results20 as $fila_generaG) {
	...
	$totalprovi = $fila_generaG -> provincia;
	  ...
	  /*contamnos cantidad de distrito*/
	   $query_dist = "SELECT count(DISTINCT distrito.nombre) AS distrito, extract(MONTH from exportacion.fnum) AS mes FROM exportacion LEFT JOIN distrito ON exportacion.ubigeo= distrito.iddistrito WHERE extract(year from exportacion.fnum)= '$anio' AND SUBSTRING(exportacion.ubigeo,1,2) = '$flatcod' AND exportacion.cpaides='$paiscode' GROUP BY mes ORDER BY mes ASC";
				/*$resultado_dist=$conexpg->query($query_dist); 
if($resultado_dist->num_rows>0){ 
while($fila_dist=$resultado_dist->fetch_array()){*/
				$query21 = $connect -> prepare($query_dist); 
$query21 -> execute(); 
$results21 = $query21 -> fetchAll(PDO::FETCH_OBJ); 
if($query21 -> rowCount() > 0)   { 
foreach($results21 as $fila_dist) {
	  ...
	  $generalH = "SELECT count(DISTINCT distrito.nombre) AS distrito FROM exportacion LEFT JOIN distrito ON exportacion.ubigeo= distrito.iddistrito WHERE extract(year from exportacion.fnum)= '$anio' AND SUBSTRING(exportacion.ubigeo,1,2) = '$flatcod' AND exportacion.cpaides='$paiscode' ";
				/*$res_generaH=$conexpg->query($generalH); 
if($res_generaH->num_rows>0){ 
while($fila_generaH=$res_generaH->fetch_array()){*/
				$query22 = $connect -> prepare($generalH); 
$query22 -> execute(); 
$results22 = $query22 -> fetchAll(PDO::FETCH_OBJ); 
if($query22 -> rowCount() > 0)   { 
foreach($results22 as $fila_generaH) {
	...
	$totaldist = $fila_generaH -> distrito;
	  ...
	  /*contamnos cantidad de agente*/
	   $query_agen = "SELECT Count(DISTINCT exportacion.cage) as cant_agen, extract(MONTH from exportacion.fnum) as mes FROM exportacion WHERE extract(year from exportacion.fnum)= '$anio' AND SUBSTRING(exportacion.ubigeo,1,2) = '$flatcod' AND exportacion.cpaides='$paiscode' GROUP BY mes order BY mes ASC";
				/*$resultado_agen=$conexpg->query($query_agen); 
if($resultado_agen->num_rows>0){ 
while($fila_agen=$resultado_agen->fetch_array()){*/
				$query23 = $connect -> prepare($query_agen); 
$query23 -> execute(); 
$results23 = $query23 -> fetchAll(PDO::FETCH_OBJ); 
if($query23 -> rowCount() > 0)   { 
foreach($results23 as $fila_agen) {
	  ...
	  $generalI = "SELECT Count(DISTINCT exportacion.cage) as cant_agen FROM exportacion WHERE extract(year from exportacion.fnum)= '$anio' AND SUBSTRING(exportacion.ubigeo,1,2) = '$flatcod' AND exportacion.cpaides='$paiscode' ";
				/*$res_generaI=$conexpg->query($generalI); 
if($res_generaI->num_rows>0){ 
while($fila_agen=$res_generaI->fetch_array()){*/
				$query24 = $connect -> prepare($generalI); 
$query24 -> execute(); 
$results24 = $query24 -> fetchAll(PDO::FETCH_OBJ); 
if($query24 -> rowCount() > 0)   { 
foreach($results24 as $fila_generaI) {
	...
	$totagente = $fila_generaI -> cant_agen;
	  ...
	  /*contamos cantidad via de transporte*/
	   $query_via = "SELECT Count(DISTINCT exportacion.cviatra) as cant_via, extract(MONTH from exportacion.fnum) as mes FROM exportacion WHERE extract(year from exportacion.fnum)= '$anio' AND SUBSTRING(exportacion.ubigeo,1,2) = '$flatcod' AND exportacion.cpaides='$paiscode' GROUP BY mes order BY mes ASC";
				/*$resultado_via=$conexpg->query($query_via); 
if($resultado_via->num_rows>0){ 
while($fila_via=$resultado_via->fetch_array()){*/
				$query25 = $connect -> prepare($query_via); 
$query25 -> execute(); 
$results25 = $query25 -> fetchAll(PDO::FETCH_OBJ); 
if($query25 -> rowCount() > 0)   { 
foreach($results25 as $fila_via) {
	  ...
	  $generalJ = "SELECT Count(DISTINCT exportacion.cviatra) as cant_via FROM exportacion WHERE extract(year from exportacion.fnum)= '$anio' AND SUBSTRING(exportacion.ubigeo,1,2) = '$flatcod' AND exportacion.cpaides='$paiscode' ";
				/*$res_generaJ=$conexpg->query($generalJ); 
if($res_generaJ->num_rows>0){ 
while($fila_generaJ=$res_generaJ->fetch_array()){*/
				$query26 = $connect -> prepare($generalJ); 
$query26 -> execute(); 
$results26 = $query26 -> fetchAll(PDO::FETCH_OBJ); 
if($query26 -> rowCount() > 0)   { 
foreach($results26 as $fila_generaJ) {
...
echo"<div class='col-md-12'>";
	...
	echo"<div class='card-header card-header-info card-header-icon'>
              ...
              <h4 class='card-title'><b>Reporte Analisis Mercado Indicador Mensual - Exportaciones </b><br> Mercado: $paisname │ Departamento: $namedepa1 │ Fecha Numeracion │ Periodo $anio </b> </h4>
            </div>";
...
<script type="text/javascript" src="https://www.gstatic.com/charts/loader.js"></script>
    <script type="text/javascript">
      ...
      function drawChart() {
        ...
        var options = {
          chart: {
            title: 'Reporte Analisis Mercado Mensual <?php echo $paisname ?> - Departamento: <?=$namedepa1;?> - Exportaciones',
