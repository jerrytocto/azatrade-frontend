...
FROM exportacion where extract(year from exportacion.fnum) >= '2012' AND extract(year from exportacion.fnum) <= '2019' AND exportacion.part_nandi=".$partida1." $codi1 ";
	}
	if($filtro=="vpesnet"){
		$sqlyear="SELECT 'vfobserdol' as VALOR, 
...
while($fila_year=$result_year->fetch_array()){
		   ...
		   if($sumatotal2017=='0' or $sumatotal2017=='' or $sumatotal2017== is_null()){
			   $varitota='0.00';
		   }else{
   ...
   SUM(CASE extract(year from exportacion.fnum) WHEN '2018' THEN exportacion.vfobserdol ELSE 0 END) AS a2018,
   SUM(CASE extract(year from exportacion.fnum) WHEN '2019' THEN exportacion.vfobserdol ELSE 0 END) AS a2019
   FROM exportacion where extract(year from exportacion.fnum) >= '2012' AND extract(year from exportacion.fnum) <= '2019' AND exportacion.part_nandi=".$partida1." $codi1 GROUP BY exportacion.dnombre";
	}
				if($filtro=="vpesnet"){
					$query1 = "SELECT exportacion.dnombre, 
   SUM(CASE extract(year from exportacion.fnum) WHEN '2012' THEN exportacion.vpesnet ELSE 0 END) AS a2012, 
...
if($nombres2=="vfobserdol" or $nombres2=="vpesnet" or $nombres2=="vpesbru" or $nombres2=="qunifis" or $nombres2=="qunicom"){
   ...
   FROM exportacion where ".$wherefecha." AND exportacion.part_nandi=".$partida1." $codi1 GROUP BY exportacion.dnombre ORDER BY a2018 DESC LIMIT 5";
  }
...
FROM exportacion
where ".$wherefecha." AND exportacion.part_nandi=".$partida1." $codi1 GROUP BY exportacion.dnombre ORDER BY a2018x DESC LIMIT 5";
  }
...
FROM exportacion
where ".$wherefecha." AND exportacion.part_nandi=".$partida1." $codi1 GROUP BY exportacion.dnombre ORDER BY a2018 DESC LIMIT 5";
  }
