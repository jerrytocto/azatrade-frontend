...
if($ubicod1!=""){
	$flatcod = $ubicod1;
	$codwe = "AND SUBSTRING(exportacion.ubigeo,1,2) = '$flatcod'";
}else{
	$flatcod = "";
	$codwe = "";
}
...
extract(year from exportacion.fnum) <= '2019' $codwe AND
...
exportacion.cpaides <> '' AND exportacion.part_nandi = ".$partida1."
GROUP BY
exportacion.cpaides,espanol,descripcion";
...
if($filtro=="vfobserdol"){
//$query1 = "SELECT exportacion.fano, exportacion.cpaides, paises.espanol, Sum(exportacion.vfobserdol) AS resultado FROM exportacion LEFT JOIN paises ON exportacion.cpaides = paises.idpaises WHERE exportacion.part_nandi = ".$partida1." AND SUBSTRING(exportacion.ubigeo,1,2) LIKE '%$flatcod' AND extract(year from exportacion.fnum) >= '2012' AND extract(year from exportacion.fnum) <= '2019' GROUP BY exportacion.cpaides,paises.espanol,exportacion.fano ORDER BY exportacion.fano,paises.espanol ASC";
$query1 = "SELECT exportacion.fano, exportacion.cpaides, '' as espanol, Sum(exportacion.vfobserdol) AS resultado FROM exportacion LEFT JOIN paises ON exportacion.cpaides = paises.idpaises WHERE exportacion.part_nandi = ".$partida1." $codwe AND extract(year from exportacion.fnum) >= '2012' AND extract(year from exportacion.fnum) <= '2019' GROUP BY exportacion.cpaides,espanol,exportacion.fano ORDER BY exportacion.fano,espanol ASC";
}
if($filtro=="vpesnet"){
	$query1 = "SELECT exportacion.fano, exportacion.cpaides, '' as espanol, Sum(exportacion.vpesnet) AS resultado FROM exportacion WHERE exportacion.part_nandi = ".$partida1." $codwe AND extract(year from exportacion.fnum) >= '2012' AND extract(year from exportacion.fnum) <= '2019' GROUP BY exportacion.cpaides,espanol,exportacion.fano ORDER BY exportacion.fano,paises.espanol ASC";
}
if($filtro=="diferen"){
	$query1 = "SELECT exportacion.fano, exportacion.cpaides, '' as espanol, (exportacion.vfobserdol/exportacion.vpesnet) AS resultado FROM exportacion WHERE exportacion.part_nandi = ".$partida1." $codwe AND extract(year from exportacion.fnum) >= '2012' AND extract(year from exportacion.fnum) <= '2019' GROUP BY exportacion.cpaides,espanol,exportacion.fano ORDER BY exportacion.fano,espanol ASC";
}
...
echo"<div class='col-md-12'>";
	...
	echo"<div class='card-header card-header-info card-header-icon'>
              ...
              <h4 class='card-title'><b>Reporte Anual de Mercado de Destino para las Exportaciones</b><br> Partida #: $partida1 │ Departamento: $namedepa1 │ Fecha Numeracion │ $nomfiltro │ Periodo 2012 - 2019 </b> </h4>
            </div>";
...
while($fila_ma=$mapa_rpt->fetch_array()){
	...
	?>
       ...
       <center>
        <h3 class="title" id="mapa"><font color="#FFFFFF">Mapa Geografico │ Partida <?=$partida1;?> │ Departamento: <?=$namedepa1;?> │ Evolucion Anual de Exportaciones │ <?=$nomfiltro;?> │ Periodo 2018</font></h3></center>
