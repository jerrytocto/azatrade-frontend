...
FROM exportacion LEFT JOIN agente ON exportacion.CAGE = agente.idagente where extract(year from exportacion.fnum) >= '2012' AND extract(year from exportacion.fnum) <= '2021' AND exportacion.part_nandi=".$partida1." $codi1 ";
	}
	if($filtro=="vpesnet"){
		$sqlyear="SELECT 'vfobserdol' as VALOR, 
...
FROM exportacion LEFT JOIN agente ON exportacion.CAGE = agente.idagente where extract(year from exportacion.fnum) >= '2012' AND extract(year from exportacion.fnum) <= '2021' AND exportacion.part_nandi=".$partida1." $codi1 GROUP BY diferen ";
				}
...
while($fila_year=$result_year->fetch_array()){
   ...
   SUM(CASE extract(year from exportacion.fnum) WHEN '2020' THEN exportacion.vfobserdol ELSE 0 END) AS a2020,
   SUM(CASE extract(year from exportacion.fnum) WHEN '2021' THEN exportacion.vfobserdol ELSE 0 END) AS a2021
   FROM exportacion LEFT JOIN agente ON exportacion.CAGE = agente.idagente where extract(year from exportacion.fnum) >= '2012' AND extract(year from exportacion.fnum) <= '2021' AND exportacion.part_nandi=".$partida1." $codi1 GROUP BY agente.agencia";
	}
				if($filtro=="vpesnet"){
					$query1 = "SELECT agente.agencia, 
   SUM(CASE extract(year from exportacion.fnum) WHEN '2012' THEN exportacion.vpesnet ELSE 0 END) AS a2012, 
...
echo"<div class='col-md-12'>";
	...
	echo"<div class='card-header card-header-info card-header-icon'>
              ...
              <h4 class='card-title'><b>Reporte Agente de Aduanas Exportadoras de Exportaciones</b><br> Partida #: $partida1 │ Departamento: $namedepa1 │ Fecha Numeracion │ $nomfiltro │ Periodo 2012 - 2021 </b> </h4>
            </div>";
		echo"<div class='card-body'>";
		echo"<div class='toolbar'>
		<a href='#agente'><button class='btn btn-info btn-sm'> Ver Estadística </button></a>
                  </div>";
...
function drawStacked() {
      var data = google.visualization.arrayToDataTable([
        ['Agente de Aduanas', '2012', '2013', '2014', '2015', '2016', '2017', '2018', '2019', '2020', '2021'],
<?php
		  include("../../incBD/inibd.php");
if($filtro=="vfobserdol" or $filtro=="vpesnet" or $filtro=="vpesbru" or $filtro=="qunifis" or $filtro=="qunicom"){
   $xquery1 = "SELECT agente.agencia, 
   ...
   FROM exportacion LEFT JOIN agente ON exportacion.CAGE = agente.idagente where ".$wherefecha." AND exportacion.part_nandi=".$partida1." $codi1 GROUP BY agente.agencia ORDER BY a2020 DESC LIMIT 5";
  //}
...
FROM exportacion LEFT JOIN agente ON exportacion.CAGE = agente.idagente
where ".$wherefecha." AND exportacion.part_nandi=".$partida1." $codi1 GROUP BY agente.agencia ORDER BY a2020x DESC LIMIT 5";
  }
...
FROM exportacion LEFT JOIN agente ON exportacion.CAGE = agente.idagente
where ".$wherefecha." AND exportacion.part_nandi=".$partida1." $codi1 GROUP BY agente.agencia ORDER BY a2020 DESC LIMIT 5";
  }
...
?>
      ...
      chart.draw(data, options);
          ...
          <div class="col-md-12 ml-auto mr-auto" id="agente">
