...
//$nombres2 = "vfobserdol";
$wherefecha = "extract(year from exportacion.fnum) >= '2012' AND extract(year from exportacion.fnum) <= '2019'";
...
FROM exportacion where extract(year from exportacion.fnum) >= '2012' AND extract(year from exportacion.fnum) <= '2019' AND exportacion.part_nandi=".$partida1." AND SUBSTRING(exportacion.ubigeo,1,2) LIKE '%$flatcod' ";
	}
	if($filtro=="vpesnet"){
		$sqlyear="SELECT 'vfobserdol' as VALOR, 
...
while($fila_year=$result_year->fetch_array()){
   ...
   SUM(CASE extract(year from exportacion.fnum) WHEN '2018' THEN exportacion.vfobserdol ELSE 0 END) AS a2018,
   SUM(CASE extract(year from exportacion.fnum) WHEN '2019' THEN exportacion.vfobserdol ELSE 0 END) AS a2019
   FROM exportacion where extract(year from exportacion.fnum) >= '2012' AND extract(year from exportacion.fnum) <= '2019' AND exportacion.part_nandi=".$partida1." AND SUBSTRING(exportacion.ubigeo,1,2) LIKE '%$flatcod' GROUP BY exportacion.dnombre";
	}
				if($filtro=="vpesnet"){
					$query1 = "SELECT exportacion.dnombre, 
   SUM(CASE extract(year from exportacion.fnum) WHEN '2012' THEN exportacion.vpesnet ELSE 0 END) AS a2012, 
...
echo"<div class='col-md-12'>";
	...
	echo"<div class='card-header card-header-info card-header-icon'>
              ...
              <h4 class='card-title'><b>Reporte Empresas Exportadoras Evolucion Anual de Exportaciones</b><br> Partida #: $partida1 │ Departamento: $namedepa1 │ Fecha Numeracion │ $nomfiltro │ Periodo 2012 - 2019 </b> </h4>
            </div>";
...
if($nombres2=="vfobserdol" or $nombres2=="vpesnet" or $nombres2=="vpesbru" or $nombres2=="qunifis" or $nombres2=="qunicom"){
   ...
   FROM exportacion where ".$wherefecha." AND exportacion.part_nandi=".$partida1." AND SUBSTRING(exportacion.ubigeo,1,2) LIKE '%$flatcod' GROUP BY exportacion.dnombre ORDER BY a2018 DESC LIMIT 5";
  }
...
FROM exportacion
where ".$wherefecha." AND exportacion.part_nandi=".$partida1." AND SUBSTRING(exportacion.ubigeo,1,2) LIKE '%$flatcod' GROUP BY exportacion.dnombre ORDER BY a2018x DESC LIMIT 5";
  }
...
FROM exportacion
where ".$wherefecha." AND exportacion.part_nandi=".$partida1." AND SUBSTRING(exportacion.ubigeo,1,2) LIKE '%$flatcod' GROUP BY exportacion.dnombre ORDER BY a2018 DESC LIMIT 5";
  }
...
?>
      ...
      var options = {
        title: 'Evolucion Anual  <?=$nomfiltro;?> de Exportaciones Departamento: <?=$namedepa1;?>  Periodo 2012 - 2019',
        chartArea: {width: '50%'},
        isStacked: true,
        hAxis: {
