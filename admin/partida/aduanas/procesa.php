...
FROM exportacion LEFT JOIN aduana ON exportacion.CADU = aduana.codadu where extract(year from exportacion.fnum) >= '2012' AND extract(year from exportacion.fnum) <= '2021' AND exportacion.part_nandi=".$partida1." $codi1 ";
	}
	if($filtro=="vpesnet"){
		$sqlyear="SELECT 'vfobserdol' as VALOR, 
...
FROM exportacion LEFT JOIN aduana ON exportacion.CADU = aduana.codadu where extract(year from exportacion.fnum) >= '2012' AND extract(year from exportacion.fnum) <= '2021' AND exportacion.part_nandi=".$partida1." $codi1 GROUP BY diferen ";
				}
...
while($fila_year=$result_year->fetch_array()){
			...
			if( $dife_precio=="diferen"){
				...
				$sumatotal2021= $sumc9 + $sumatotal2021;
   ...
   SUM(CASE extract(year from exportacion.fnum) WHEN '2020' THEN exportacion.vfobserdol ELSE 0 END) AS a2020,
   SUM(CASE extract(year from exportacion.fnum) WHEN '2021' THEN exportacion.vfobserdol ELSE 0 END) AS a2021
   FROM exportacion LEFT JOIN aduana ON exportacion.CADU = aduana.codadu where extract(year from exportacion.fnum) >= '2012' AND extract(year from exportacion.fnum) <= '2021' AND exportacion.part_nandi=".$partida1." $codi1 GROUP BY aduana.descripcion";
	}
				if($filtro=="vpesnet"){
					$query1 = "SELECT aduana.descripcion, 
   SUM(CASE extract(year from exportacion.fnum) WHEN '2012' THEN exportacion.vpesnet ELSE 0 END) AS a2012, 
...
echo"<div class='col-md-12'>";
	...
	echo"<div class='card-header card-header-info card-header-icon'>
              ...
              <h4 class='card-title'><b>Reporte de Aduanas Exportadoras de Exportaciones</b><br> Partida #: $partida1 │ Departamento: $namedepa1 │ Fecha Numeración │ $nomfiltro │ Periodo 2012 - 2021 </b> </h4>
            </div>";
		echo"<div class='card-body'>";
		echo"<div class='toolbar'>
		<a href='#aduana'><button class='btn btn-info btn-sm'> Ver Estadística </button></a>
                  </div>";
		...
		echo"<table id='datatables' class='table table-striped table-no-bordered table-hover' cellspacing='0' width='100%' style='width:100%'>";
		 ...
		 echo"<tfoot>
		  ...
		  if($dife_deta=="diferen"){
			  ...
			  if($dife2021x == "0" or $dife2021x == "0.00"){
				  ...
				  $year11A = $dife2021 / $dife2021x;
				}
				
		  }else{
		  $year3= number_format($fila1['a2012'],2);
		  $year4= number_format($fila1['a2013'],2);
		  $year5= number_format($fila1['a2014'],2);
		  $year6= number_format($fila1['a2015'],2);
		  $year7= number_format($fila1['a2016'],2);
		  $year8= number_format($fila1['a2017'],2);
		  $year9= number_format($fila1['a2018'],2);
		  $year10= number_format($fila1['a2019'],2);
			  $year10A= $fila1['a2019'];
			  $year11= number_format($fila1['a2020'],2);
			  $year11A= $fila1['a2020'];
			  $year12= number_format($fila1['a2021'],2);
		  }
		 
		 if($year10=='0'){
		  $var11 ='0';
		  }else{
		  $var11 = number_format((($year11A / $year10A) - 1) * 100,2);
		  }
		 ...
		 if($sumatotal2020=='0'){
		  ...
		  $var22 = number_format(($year11A / $sumatotal2020) * 100,2);
		  }
...
if($filtro=="vfobserdol" or $filtro=="vpesnet" or $filtro=="vpesbru" or $filtro=="qunifis" or $filtro=="qunicom"){
   ...
   FROM exportacion LEFT JOIN aduana ON exportacion.CADU = aduana.codadu where ".$wherefecha." AND exportacion.part_nandi=".$partida1." $codi1 GROUP BY aduana.descripcion ORDER BY a2021 DESC LIMIT 5";
  //}
...
FROM exportacion LEFT JOIN aduana ON exportacion.CADU = aduana.codadu
where ".$wherefecha." AND exportacion.part_nandi=".$partida1." $codi1 GROUP BY aduana.descripcion ORDER BY a2020x DESC LIMIT 5";
  }
...
FROM exportacion LEFT JOIN aduana ON exportacion.CADU = aduana.codadu
where ".$wherefecha." AND exportacion.part_nandi=".$partida1." $codi1 GROUP BY aduana.descripcion ORDER BY a2020 DESC LIMIT 5";
  }
...
?>
      ...
      chart.draw(data, options);
          ...
          <div class="col-md-12 ml-auto mr-auto" id="aduana">
