...
extract(year from exportacion.fnum) <= '2021' $query1 AND exportacion.part_nandi='".$bus2."'";
...
$var3= number_format((($vpes_2020/$vpes_2019) - 1) * 100,2);
	...
	if($vpes_2020=='0' or $vpes_2020==null){
			  ...
			  $var4= number_format(($ca1x + $ca2x + $ca3x + $ca4x + $ca5x + $ca6x + $ca6x + $ca7x)/8,2) ;
...
/*sumamos unidades comerciales*/
	  ...
	  FROM exportacion where extract(year from exportacion.fnum) >= '2012' AND
extract(year from exportacion.fnum) <= '2020' $query1 AND exportacion.part_nandi='".$bus2."'";
...
extract(year from exportacion.fnum) <= '2021' $query1 AND exportacion.part_nandi='".$bus2."' GROUP BY anio order BY anio ASC";
...
while($fila_via=$resultado_via->fetch_array()){
	...
	if($rvia2019=='0' or $rvia2019 == null){
		      ...
		      $ca7_11 = (($rvia2020 / $rvia2019) - 1) * 100;
		      }
	if($rvia2020=='0' or $rvia2020 == null){
			  $ca8_11 ='0';
		      }else{
		      $ca8_11 = (($rvia2021 / $rvia2020) - 1) * 100;
		      }
...
echo"<div class='col-md-12'>";
	...
	echo"<div class='card-header card-header-info card-header-icon'>
              ...
              <h4 class='card-title'><b>Reporte Anual de Indicadores</b><br> Partida #: $bus2 │ Departamento: $namedepa1 │ Fecha Numeración │ Valor FOB USD │ Periodo 2012 - 2021 </b> </h4>
            </div>";
		...
		<td><form method='post' action='../indicador-mensual/' target='_blank'>
										<input type='hidden' name='partidaindimen' value='$bus2' >
										<input type='hidden' name='codubi' value='$ubicod1'>
				                        <input type='hidden' name='namedepa' value='$namedepa1'>
										<input type='hidden' name='year' value='2012'>
								<center><button type='submit' class='btn-link'>Ver Mes</button></center>
										</form></td>
		<td><form method='post' action='../indicador-mensual/' target='_blank'>
										<input type='hidden' name='partidaindimen' value='$bus2' >
										<input type='hidden' name='codubi' value='$ubicod1'>
      ...
      function drawChart() {
        ...
        var options = {
          chart: {
            ...
            subtitle: 'Cuadro Estadístico, Partida: <?=$bus2;?> - Departamento: <?=$namedepa1;?> - Periodo: 2012-2021',
          },
          bars: 'vertical',
