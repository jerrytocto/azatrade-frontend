...
//seleccionamos y del resultado insertamos a la tabla temporal*/
  ...
  $query1 = "SELECT exportacion.fano, extract(MONTH from exportacion.fnum) as mes, sum(exportacion.vfobserdol)/sum(exportacion.vpesnet) as resultado FROM exportacion WHERE exportacion.part_nandi = ".$partida1." $query1var AND extract(year from exportacion.fnum) >= '2012' AND extract(year from exportacion.fnum) <= '2021' GROUP BY exportacion.fano,mes ORDER BY exportacion.fano,mes";
				...
				if($indica=="cantemp"){
  $query1 = "SELECT DISTINCT count(exportacion.ndoc) as resultado, exportacion.fano, extract(MONTH from exportacion.fnum) as mes FROM exportacion WHERE exportacion.part_nandi = ".$partida1." $query1var AND extract(year from exportacion.fnum) >= '2012' AND extract(year from exportacion.fnum) <= '2021' GROUP BY exportacion.fano,mes ORDER BY exportacion.fano,mes";
					}
				if($indica=="cantmerca"){
...
while($fila1=$resultado_insercons->fetch_array()){
 ...
 $rs = $conexpg->query($insert1);
	...
	//consultamos suma total del precio		 
	if($indica=="diferen"){//precio suma totales por año
	$preciosqlxx = "SELECT exportacion.fano, sum(exportacion.vfobserdol)/sum(exportacion.vpesnet) as resultado  FROM exportacion  WHERE exportacion.part_nandi = ".$partida1." AND  extract(year from exportacion.fnum) >= '2012' AND  extract(year from exportacion.fnum) <= '2021'  GROUP BY exportacion.fano  ORDER BY exportacion.fano";
...
while($fila_rptx=$resultado_rptx->fetch_array()){
        ...
        var options = {
          title: 'Partida: <?php echo $partida1 ?> / Departamento: <?=$namedepa1;?> / Evolucion Mensual de Exportaciones / Valor FOB USD / Periodo: 2012 - 2021',
          /*curveType: 'function',
