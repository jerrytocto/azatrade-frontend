...
if($varche=="1" and $wv6==""){//si esta marcado y es vacio
	...
	$siejecuta="si";
}else if($wv1=="" or $vardepa=="" and $wv3=="" and $wv4==""){
	$mensajeB="error2";
}else if($wv1!="" and $vardepa!="" or $wv3!="" or $wv4!=""){
	$siejecuta="si";
}
?>
<!DOCTYPE html>
...
<Center><img class="img-responsive" src="img/loading-carga.gif" width="100px"><br><p style="font-size:12px; color:#EC0D36;"> Espere un momento, la consulta puede tardar unos minutos. </p></Center>
        ...
        <?php if($mensajeA=="error1"){ ?>
           ...
           <div class="alert alert-warning">
            ...
            <!--<span><b> Mensaje - </b> debe de seleccionar un Año, Mes y Departamento como campos REQUERIDOS</span>-->
            <span><b> Mensaje - </b> debe de seleccionar un Año, Mes y Departamento o Pais o Aduanas como campos REQUERIDOS</span>
          </div>
           <?php } ?>
...
//}
					...
					{ 
						...
						echo "<option value='$filaxd[0]' "; if($_POST["depa"]==$filaxd[0]){echo "selected";} echo " > $filaxd[1] </option>";
					}
					echo "</select>";?>                     
                                               ...
                                               <?php $daa=$_POST['ruce']; ?>
                                                <div class="form-group">
                                                   <!--<label for="exampleEmail" class="bmd-label-floating">Ruc Empresa</label>-->
                                                    <input type="text" name="ruce" class="form-control css-input"  value="<?php echo"$daa";?>"  onKeyPress="return justNumbers(event);" placeholder="Ingrese N#. Ruc Empresa" />
                                                </div>
                                            </div> 
                                             <div class="col-md-3">
                                                <div class="form-group">
                    ...
                    <span class="form-check-sign">
                      <span class="check"></span>
                                            ...
                                            <div class="col-md-3" id="ocultable" <?=$ocu;?> >
                                                <div class="form-group">
                                                <input type="text" name="numpar" class="form-control css-input" placeholder="Ingrese N#. Partida" value="<?php echo"$de2";?>" onKeyPress="return justNumbers(event);" />
                                            ...
                                            <div class="col-md-12">
                                               <?php $de1=$_POST['descri1']; ?>
                                                <div class="form-group">
                                                    <input type="text" name="descri1" class="form-control css-input" placeholder="Descripci&oacute;n del Producto" value="<?php echo"$de1";?>"  />
                                                </div>
                                            </div>
...
$like="(coalesce(e.dcom,'') ||' '|| coalesce(e.dmer2,'') ||' '|| coalesce(e.dmer3,'') ||' '|| coalesce(e.dmer4,'') ||' '|| coalesce(e.dmer5,'') ilike '".$campode."')";
	...
	//consultamos cantidad de registro por el año seleccionado
	$cuenreg = "SELECT count(e.PART_NANDI) as totalreg FROM exportacion AS e WHERE YEAR(".$wherefecha1.") = '".$year."' and e.PART_NANDI like '%".$partida."%' and e.CADU like '%".$aduana."%' and e.NDOC like '%".$empresa."%' and substring(e.UBIGEO,1,2) like '%".$depa."%' and CONCAT(e.DCOM,' ',e.DMER2,' ',e.DMER3,' ',e.DMER4,' ',e.DMER5) like '%".$campode."%' and e.CPAIDES like '%".$pais."%' ";	 
	$res_creg=$conexpg->query($cuenreg); 
if($res_creg->num_rows>0){
	while($fila_cureg=$res_creg->fetch_array()){
		$totareg = $fila_cureg['totalreg'];
	}
}else{
	$totareg = "0";
}
...
while($rwage=$rsage->fetch_array()){ 
   ...
   echo '<td>'.$nom_sector.'</td>';
	...
	echo "<p align='center'>";
			if($itte<="300000"){
			 echo "<a href='descarga_detalladotodopartida.php?numpar=$partida&region=$depa&country=$pais&bus=$campode&emp=$empresa&aduana=$aduana' target='_blank' class='btn btn-fill btn-success'><i class='material-icons'>cloud_download</i> Descargar Data de todo los años</a>";
			 }else{
			echo"<a href='#' data-toggle='modal' data-target='#myModallimita' class='btn btn-fill btn-success'><i class='material-icons'>cloud_download</i> Descargar Data de todo los años</a>"; 
			 }
	echo "<a href='descarga_detalladotodopartida.php?numpar=$partida&region=$depa&country=$pais&bus=$campode&emp=$empresa&aduana=$aduana&selecanios=$year' target='_blank' class='btn btn-fill btn-primary'><i class='material-icons'>cloud_download</i> Descargar Data de todo el año: $year </a>";
	/*if($totareg<='5'){
	echo "<a href='descarga_detalladotodopartida.php?numpar=$partida&region=$depa&country=$pais&bus=$campode&emp=$empresa&aduana=$aduana&selecanios=$year' target='_blank' class='btn btn-fill btn-primary'><i class='material-icons'>cloud_download</i> Descargar Data de todo el año: $year </a>";
	}else{
		echo"<a href='#' data-toggle='modal' data-target='#Dosmodalanio' class='btn btn-fill btn-primary'><i class='material-icons'>cloud_download</i> Descargar Data de todo el año: $year</a>"; 
	}*/
	echo "</p>";
	
				//modal1
echo"<div class='modal fade' id='myModallimita' role='dialog'>
      ...
      <div class='modal-content'>
        ...
        <div class='modal-body'>
                    					<p>Esta descarga contiene mas de 300,000 mil registros; se limitara solo a descargar su limite m&aacute;ximo que es los 300,000 mil registros maximos permitidos, utilizando los filtros seleccionados</p>
                    					<p align='center'><a href='descarga_detalladotodopartida.php?numpar=$partida&region=$depa&country=$pais&bus=$campode&emp=$empresa&aduana=$aduana' target='_blank' class='btn btn-fill btn-success'><i class='material-icons'>cloud_download</i> Descargar Data de todo los años</a></p>
                    				</div>
