...
WHERE exportacion.part_nandi = ".$partida1." AND CONCAT(exportacion.dcom,exportacion.dmer2,exportacion.dmer3,exportacion.dmer4,exportacion.DMER5) LIKE '%$describus%'  
...
extract(year from exportacion.fnum) <= '2021' AND CONCAT(exportacion.dcom,exportacion.dmer2,exportacion.dmer3,exportacion.dmer4,exportacion.DMER5) LIKE '%$describus%' GROUP BY anio,departamento.nombre,ubi1 ORDER BY anio,nombre ASC";
	}
if($filtro=="vpesnet"){
...
if($filtro=="diferen"){
	$query1 = "SELECT extract(YEAR from exportacion.fnum) AS anio, substring(exportacion.ubigeo,1,2) as ubi1, departamento.nombre, (exportacion.vfobserdol/exportacion.vpesnet) AS resultado FROM exportacion LEFT JOIN departamento ON   ((((SUBSTRING(exportacion.ubigeo,1,2))= departamento.iddepartamento))) WHERE exportacion.part_nandi = ".$partida1." AND extract(YEAR from exportacion.fnum) >= '2012' AND
extract(year from exportacion.fnum) <= '2021' AND CONCAT(exportacion.dcom,exportacion.dmer2,exportacion.dmer3,exportacion.dmer4,exportacion.DMER5) LIKE '%$describus%' GROUP BY anio,departamento.nombre,ubi1 ORDER BY anio,nombre ASC";
}
...
if($querya!=0){//inicia si hay resultados
	/*if($filtro=="vfobserdol"){
	$queryr = "SELECT extract(YEAR from exportacion.fnum) AS anio, Sum(exportacion.vfobserdol) AS resultado FROM exportacion LEFT JOIN departamento ON   ((((SUBSTRING(exportacion.ubigeo,1,2))= departamento.iddepartamento))) WHERE exportacion.part_nandi = ".$partida1." AND extract(year from exportacion.fnum) = '2019' AND CONCAT(exportacion.dcom,exportacion.dmer2,exportacion.dmer3,exportacion.dmer4,exportacion.DMER5) LIKE '%$describus%' GROUP BY anio order BY anio ASC";
		}
	if($filtro=="vpesnet"){
	...
	$querrx=$conexpg->query($queryr); 
if($querrx->num_rows>0){ 
while($fila_rx=$querrx->fetch_array()){ 
...
if($result_year->num_rows>0){
	...
	$qusr=$conexpg->query($qresultadosr); 
if($qusr->num_rows>0){
	...
	echo"<div class='card-header card-header-info card-header-icon'>
              ...
              <h4 class='card-title'><b>Reporte de Ubigeo Anual de Exportaciones</b><br> Producto: $describus │ Partida #: $partida1 │ Departamento: Todos │ Fecha Numeracion │ $nomfiltro │ Periodo 2012 - 2021 </b> </h4>
            </div>";
	...
	while($fila_rpt=$qusr->fetch_array()){ 
		...
		if($tota8=='0'){
		  ...
		  $parti15=number_format(($mes9 / $sumatotal2019) * 100,2);
			  }
		  }
